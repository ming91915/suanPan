...
typedef struct h5tool_format_t {
     ...
     *
     *   listv:    h5watch: vector containing info about the list of compound fields to be
     * printed.
     */
...
typedef struct h5tools_context_t {
    ...
    struct H5LD_memb_t**
        cmpd_listv; /* h5watch: vector containing info about the list of compound fields
                       to be printed */
} h5tools_context_t;
...
H5TOOLS_DLLVAR unsigned long long
    packed_data_mask;                /* mask in which packed bits to display */
...
H5TOOLS_DLL int
render_bin_output(FILE* stream, hid_t container, hid_t tid, void* _mem, hsize_t nelmts);
