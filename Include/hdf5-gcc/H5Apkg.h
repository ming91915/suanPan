...
/* Define the 'found' callback function pointer for matching an attribute record in a v2
 * B-tree */
typedef herr_t (
    *H5A_bt2_found_t)(const H5A_t* attr, hbool_t* took_ownership, void* op_data);
...
H5_DLL htri_t H5A_exists_by_name(H5G_loc_t loc,
    ...
    hid_t lapl_id,
    hid_t dxpl_id);
H5_DLL herr_t H5A__write(H5A_t* attr,
    const H5T_t* mem_type,
    const void* buf,
    hid_t dxpl_id);
H5_DLL herr_t H5A__read(const H5A_t* attr,
    const H5T_t* mem_type,
    void* buf,
    hid_t dxpl_id);
