                ...
                for(i = 0, j = 1; j < n_rows; i += 2, j += 2) {     \
                    ...
                    *out_mem operatorA tmp_j;                       \
                    out_mem++;                                      \
                                                                    ...
                                                                    \
                if(i < n_rows) {                                    \
                    *out_mem operatorA eop_core<eop_type>::process( \
                        P.at(i, col, slice), k);                    \
                    out_mem++;                                      \
                }                                                   \
            }                                                       \
...
arma_hot inline void eop_core<eop_type>::apply_inplace_plus(
...
template <typename eop_type>
template <typename T1>
arma_hot inline void eop_core<eop_type>::apply_inplace_minus(
    Mat<typename T1::elem_type>& out,
    const eOp<T1, eop_type>& x)
{
    arma_extra_debug_sigprint();
...
template <typename eop_type>
template <typename T1>
arma_hot inline void eop_core<eop_type>::apply_inplace_schur(
    Mat<typename T1::elem_type>& out,
    const eOp<T1, eop_type>& x)
{
    arma_extra_debug_sigprint();
...
template <typename eop_type>
template <typename T1>
arma_hot inline void eop_core<eop_type>::apply_inplace_div(
    Mat<typename T1::elem_type>& out,
    const eOp<T1, eop_type>& x)
{
    arma_extra_debug_sigprint();
...
template <typename eop_type>
template <typename T1>
arma_hot inline void eop_core<eop_type>::apply(Cube<typename T1::elem_type>& out,
    const eOpCube<T1, eop_type>& x)
{
    ...
    const bool use_mp = (arma_config::cxx11 && arma_config::openmp) &&
        (eOpCube<T1, eop_type>::use_mp || (is_same_type<eop_type, eop_pow>::value &&
                                              (is_cx<eT>::yes || x.aux != eT(2))));
