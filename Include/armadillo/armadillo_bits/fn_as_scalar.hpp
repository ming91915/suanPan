...
inline typename T1::elem_type as_scalar_redirect<N>::apply(const T1& X)
{
    ...
    if(P.get_n_elem() != 1) {
        arma_debug_check(
            true, "as_scalar(): expression doesn't evaluate to exactly one element");
    ...
    const Glue<Glue<T1, T2, glue_times>, T3, glue_times>& X)
{
    ...
    if(tmp2_do_diagmat == false) {
        ...
        const uword B_n_rows = (B_is_vec == true) ?
            B.n_elem :
            ((tmp2.do_trans == false) ? B.n_rows : B.n_cols);
        const uword B_n_cols = (B_is_vec == true) ?
            B.n_elem :
    ...
    const Glue<Glue<T1, T2, glue_times_diag>, T3, glue_times>& X)
{
    ...
    const uword B_n_rows =
        (B_is_vec == true) ? B.n_elem : ((tmp2.do_trans == false) ? B.n_rows : B.n_cols);
    const uword B_n_cols =
        (B_is_vec == true) ? B.n_elem : ((tmp2.do_trans == false) ? B.n_cols : B.n_rows);
...
template <typename T1, typename T2>
arma_warn_unused arma_inline typename T1::elem_type as_scalar(
    const Glue<T1, T2, glue_times>& X,
...
inline arma_warn_unused typename T1::elem_type as_scalar(
...
template <typename T>
arma_warn_unused arma_inline const typename arma_scalar_only<T>::result& as_scalar(
    const T& x)
{
    return x;
}
