...
inline Mat<eT>::Mat(const uword in_n_rows,
...
//! constructor used by Row and Col classes
template <typename eT>
inline Mat<eT>::Mat(const arma_vec_indicator&,
...
template <typename eT>
inline Mat<eT>::Mat(const arma_fixed_indicator&,
    const uword in_n_rows,
    const uword in_n_cols,
    const uhword in_vec_state,
    const eT* in_mem)
...
template <typename eT> inline void Mat<eT>::init_cold()
{
    arma_extra_debug_sigprint(
        arma_str::format("n_rows = %d, n_cols = %d") % n_rows % n_cols);
...
inline void Mat<eT>::init(const std::initializer_list<std::initializer_list<eT>>& list)
{
    ...
    for(; it != it_end; ++it) {
        if(x_n_cols_found == false) {
            ...
            arma_check((uword((*it).size()) != x_n_cols),
                "Mat::init(): inconsistent number of columns in initialiser list");
        }
    }
    ...
    const Base<typename Mat<eT>::pod_type, T2>& Y)
{
    ...
    arma_type_check((is_complex<eT>::value ==
        false)); //!< compile-time abort if eT is not std::complex
    arma_type_check(
        (is_complex<T>::value == true)); //!< compile-time abort if  T is     std::complex
...
template <typename eT> inline void Mat<eT>::steal_mem(Mat<eT>& x)
{
    ...
    if((t_mem_state <= 1) &&
        (((x_mem_state == 0) && (x_n_elem > arma_config::mat_prealloc)) ||
            (x_mem_state == 1)) &&
        layout_ok) {
        reset();
...
template <typename eT>
inline Mat<eT>::Mat(const char junk,
    const eT* aux_mem,
    const uword aux_n_rows,
    const uword aux_n_cols)
    : n_rows(aux_n_rows)
    ...
    if(in_n_slices == 1) {
        ...
        if(out_vec_state == 0) {
            if((in_n_rows == out_n_rows) && (in_n_cols == 1) &&
                ...
                for(uword i = 0; i < in_n_slices; ++i) {
                    arrayops::inplace_plus(
                        out.colptr(i), in.slice_colptr(i, 0), in_n_rows);
                }
    ...
    if(in_n_slices == 1) {
        for(uword ucol = 0; ucol < in_n_cols; ++ucol) {
            arrayops::inplace_minus(
                out.colptr(ucol), in.slice_colptr(0, ucol), in_n_rows);
        }
    } else {
        if(out_vec_state == 0) {
            if((in_n_rows == out_n_rows) && (in_n_cols == 1) &&
...
inline Mat<eT>& Mat<eT>::operator+=(const SpBase<eT, T1>& m)
{
    ...
    arma_debug_assert_same_size(
        n_rows, n_cols, p.get_n_rows(), p.get_n_cols(), "addition");
...
inline Mat<eT>& Mat<eT>::operator-=(const SpBase<eT, T1>& m)
{
    ...
    arma_debug_assert_same_size(
        n_rows, n_cols, p.get_n_rows(), p.get_n_cols(), "subtraction");
...
inline Mat<eT>& Mat<eT>::operator/=(const SpBase<eT, T1>& m)
{
    ...
    arma_debug_assert_same_size(
        n_rows, n_cols, p.get_n_rows(), p.get_n_cols(), "element-wise division");
...
template <typename eT>
inline const subview<eT> Mat<eT>::
...
template <typename eT>
template <typename T1, typename T2>
arma_inline const subview_elem2<eT, T1, T2> Mat<eT>::elem(const Base<uword, T1>& ri,
...
template <typename eT>
template <typename T1, typename T2>
arma_inline const subview_elem2<eT, T1, T2> Mat<eT>::submat(const Base<uword, T1>& ri,
    const Base<uword, T2>& ci) const
{
    arma_extra_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2>
arma_inline const subview_elem2<eT, T1, T2> Mat<eT>::operator()(const Base<uword, T1>& ri,
    const Base<uword, T2>& ci) const
{
    arma_extra_debug_sigprint();
...
inline void Mat<eT>::swap_rows(const uword in_row1, const uword in_row2)
{
    ...
    arma_debug_check((in_row1 >= local_n_rows) || (in_row2 >= local_n_rows),
...
inline void Mat<eT>::swap_cols(const uword in_colA, const uword in_colB)
{
    ...
    arma_debug_check((in_colA >= local_n_cols) || (in_colB >= local_n_cols),
...
inline Mat<eT>& Mat<eT>::operator=(const eGlue<T1, T2, eglue_type>& X)
{
    ...
    const bool bad_alias = ((eGlue<T1, T2, eglue_type>::proxy1_type::has_subview &&
                                X.P1.is_alias(*this)) ||
...
//! linear element accessor (treats the matrix as a vector); no bounds check.
template <typename eT> arma_inline arma_warn_unused eT& Mat<eT>::at(const uword ii)
{
    return access::rw(mem[ii]);
}
...
//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline arma_warn_unused const eT& Mat<eT>::operator()(const uword in_row,
...
//! element accessor; no bounds check
template <typename eT>
arma_inline arma_warn_unused const eT& Mat<eT>::at(const uword in_row,
    const uword in_col) const
{
    return mem[in_row + in_col * n_rows];
}
...
template <typename eT> arma_inline arma_warn_unused bool Mat<eT>::is_empty() const
{
    return (n_elem == 0);
}

//! returns true if the object can be interpreted as a column or row vector
template <typename eT> arma_inline arma_warn_unused bool Mat<eT>::is_vec() const
{
    return ((n_rows == 1) || (n_cols == 1));
}

//! returns true if the object can be interpreted as a row vector
template <typename eT> arma_inline arma_warn_unused bool Mat<eT>::is_rowvec() const
{
    return (n_rows == 1);
}

//! returns true if the object can be interpreted as a column vector
template <typename eT> arma_inline arma_warn_unused bool Mat<eT>::is_colvec() const
{
    return (n_cols == 1);
}

//! returns true if the object has the same number of non-zero rows and columnns
template <typename eT> arma_inline arma_warn_unused bool Mat<eT>::is_square() const
{
    return (n_rows == n_cols);
}

//! returns true if all of the elements are finite
template <typename eT> inline arma_warn_unused bool Mat<eT>::is_finite() const
{
...
template <typename eT>
inline arma_warn_unused bool Mat<eT>::is_sorted(const char* direction,
    const uword dim) const
{
    ...
    arma_debug_check(
        ((sig != 'a') && (sig != 'd')), "Mat::is_sorted(): unknown sort direction");
...
template <typename eT> arma_inline arma_warn_unused eT* Mat<eT>::memptr()
{
    return const_cast<eT*>(mem);
}

//! returns a pointer to array of eTs used by the matrix
template <typename eT> arma_inline arma_warn_unused const eT* Mat<eT>::memptr() const
{
    return mem;
}
...
//! on return, the stream's state are restored to their original values.
template <typename eT>
inline void Mat<eT>::impl_print(std::ostream& user_stream,
...
//! (i.e. the precision and cell width are not modified).
template <typename eT>
inline void Mat<eT>::impl_raw_print(std::ostream& user_stream,
    const std::string& extra_text) const
{
    arma_extra_debug_sigprint();
...
template <typename eT> arma_hot inline const Mat<eT>& Mat<eT>::fill(const eT val)
{
    arma_extra_debug_sigprint();
...
template <typename eT> inline void Mat<eT>::row_iterator::operator++(int)
{
    operator++();
}
...
template <typename eT> inline eT Mat<eT>::const_row_iterator::operator*() const
{
    return M.at(row, col);
}
...
template <typename eT> inline void Mat<eT>::const_row_iterator::operator++(int)
{
    operator++();
}
...
template <typename eT> inline const eT& Mat<eT>::const_row_col_iterator::operator*() const
{
    return *current_pos;
}
...
template <typename eT> inline uword Mat<eT>::const_row_col_iterator::row() const
{
    return internal_row;
}

template <typename eT> inline uword Mat<eT>::const_row_col_iterator::col() const
{
    return internal_col;
}
...
template <typename eT> inline typename Mat<eT>::row_col_iterator Mat<eT>::begin_row_col()
{
...
template <typename eT> inline typename Mat<eT>::row_col_iterator Mat<eT>::end_row_col()
{
...
{
    return Op<
        typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type,
        op_htrans>(*this);
}
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline arma_warn_unused const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at_alt(
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline arma_warn_unused eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator[](
    const uword ii)
{
    return (use_extra) ? mem_local_extra[ii] : mem_local[ii];
}
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline arma_warn_unused const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::
operator[](const uword ii) const
{
    return (use_extra) ? mem_local_extra[ii] : mem_local[ii];
}
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline arma_warn_unused eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(
    const uword ii)
{
    return (use_extra) ? mem_local_extra[ii] : mem_local[ii];
}
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline arma_warn_unused const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(
    const uword ii) const
{
    return (use_extra) ? mem_local_extra[ii] : mem_local[ii];
}
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline arma_warn_unused eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::
...
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline arma_warn_unused const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::
operator()(const uword in_row, const uword in_col) const
{
    arma_debug_check(((in_row >= fixed_n_rows) || (in_col >= fixed_n_cols)),
        "Mat::operator(): index out of bounds");
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline arma_warn_unused eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::colptr(
    const uword in_col)
{
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline arma_warn_unused const eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::colptr(
    const uword in_col) const
{
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_hot inline const Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fill(
    const eT val)
{
    arma_extra_debug_sigprint();
...
inline void Mat_aux::set_real(Mat<std::complex<T>>& out, const Base<T, T1>& X)
{
    ...
    arma_debug_assert_same_size(
        out.n_rows, out.n_cols, local_n_rows, local_n_cols, "Mat::set_real()");
...
inline void Mat_aux::set_imag(Mat<std::complex<T>>& out, const Base<T, T1>& X)
{
    ...
    arma_debug_assert_same_size(
        out.n_rows, out.n_cols, local_n_rows, local_n_cols, "Mat::set_imag()");
