    ...
    const Base<eT, T3>& in_hefts_expr)
{
    ...
    arma_debug_check((in_means.is_finite() == false),
        "gmm_diag::set_params(): given means have non-finite values");
    arma_debug_check((in_dcovs.is_finite() == false),
        "gmm_diag::set_params(): given dcovs have non-finite values");
    arma_debug_check((in_hefts.is_finite() == false),
    ...
    arma_debug_check(((s < (eT(1) - Datum<eT>::eps)) || (s > (eT(1) + Datum<eT>::eps))),
...
inline void gmm_diag<eT>::set_means(const Base<eT, T1>& in_means_expr)
{
    ...
    arma_debug_check((size(in_means) != size(means)),
...
inline void gmm_diag<eT>::set_dcovs(const Base<eT, T1>& in_dcovs_expr)
{
    ...
    arma_debug_check((size(in_dcovs) != size(dcovs)),
...
inline void gmm_diag<eT>::set_hefts(const Base<eT, T1>& in_hefts_expr)
{
    ...
    arma_debug_check((size(in_hefts) != size(hefts)),
...
template <typename eT>
template <typename T1>
inline eT gmm_diag<eT>::log_p(const T1& expr,
    const gmm_empty_arg& junk1,
    typename enable_if<((is_arma_type<T1>::value) &&
        (resolves_to_colvector<T1>::value == true))>::result* junk2) const
{
    arma_extra_debug_sigprint();
    arma_ignore(junk1);
    arma_ignore(junk2);
    ...
    arma_debug_check(
        (tmp.M.n_rows != means.n_rows), "gmm_diag::log_p(): incompatible dimensions");
...
template <typename eT>
template <typename T1>
inline Row<eT> gmm_diag<eT>::log_p(const T1& expr,
    const gmm_empty_arg& junk1,
    typename enable_if<((is_arma_type<T1>::value) &&
        (resolves_to_colvector<T1>::value == false))>::result* junk2) const
{
    arma_extra_debug_sigprint();
    arma_ignore(junk1);
    arma_ignore(junk2);
...
template <typename eT>
template <typename T1>
inline uword gmm_diag<eT>::assign(const T1& expr,
    const gmm_dist_mode& dist,
    typename enable_if<((is_arma_type<T1>::value) &&
        (resolves_to_colvector<T1>::value == true))>::result* junk) const
{
    arma_extra_debug_sigprint();
    arma_ignore(junk);
...
template <typename eT>
template <typename T1>
inline urowvec gmm_diag<eT>::assign(const T1& expr,
    const gmm_dist_mode& dist,
    typename enable_if<((is_arma_type<T1>::value) &&
        (resolves_to_colvector<T1>::value == false))>::result* junk) const
{
    arma_extra_debug_sigprint();
    arma_ignore(junk);
    ...
    const gmm_dist_mode& dist_mode) const
{
    ...
    arma_debug_check(
        (X.n_rows != means.n_rows), "gmm_diag::raw_hist(): incompatible dimensions");
    ...
    const gmm_dist_mode& dist_mode) const
{
    ...
    arma_debug_check(
        (X.n_rows != means.n_rows), "gmm_diag::norm_hist(): incompatible dimensions");
...
template <typename eT>
template <typename T1>
inline bool gmm_diag<eT>::learn(const Base<eT, T1>& data,
    const uword N_gaus,
    ...
    const bool print_mode)
{
    ...
    const bool seed_mode_ok = (seed_mode == keep_existing) ||
        (seed_mode == static_subset) || (seed_mode == static_spread) ||
    ...
    arma_debug_check(
        (var_floor < eT(0)), "gmm_diag::learn(): variance floor is negative");
...
template <typename eT>
template <typename T1>
inline bool gmm_diag<eT>::kmeans_wrapper(Mat<eT>& user_means,
...
inline Row<eT> gmm_diag<eT>::internal_vec_log_p(const T1& X) const
{
    ...
    arma_debug_check(
        (X.n_rows != means.n_rows), "gmm_diag::log_p(): incompatible dimensions");
...
inline eT gmm_diag<eT>::internal_sum_log_p(const T1& X) const
{
    ...
    arma_debug_check(
        (X.n_rows != means.n_rows), "gmm_diag::sum_log_p(): incompatible dimensions");
...
inline eT gmm_diag<eT>::internal_avg_log_p(const T1& X) const
{
    ...
    arma_debug_check(
        (X.n_rows != means.n_rows), "gmm_diag::avg_log_p(): incompatible dimensions");
...
template <typename eT>
template <typename T1>
inline uword gmm_diag<eT>::internal_scalar_assign(const T1& X,
    const gmm_dist_mode& dist_mode) const
{
    arma_extra_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void gmm_diag<eT>::internal_vec_assign(urowvec& out,
    const T1& X,
    const gmm_dist_mode& dist_mode) const
{
    arma_extra_debug_sigprint();
...
template <typename eT>
inline void gmm_diag<eT>::internal_raw_hist(urowvec& hist,
    const Mat<eT>& X,
    const gmm_dist_mode& dist_mode) const
{
    arma_extra_debug_sigprint();
...
template <typename eT>
template <uword dist_id>
inline bool gmm_diag<eT>::km_iterate(const Mat<eT>& X,
...
#pragma omp parallel for schedule(static)
        ...
        for(uword g = 0; g < N_gaus; ++g) {
            rs_delta(distance<eT, dist_id>::eval(
                N_dims, old_means.colptr(g), new_means.colptr(g), mah_aux_mem));
        }
...
template <typename eT>
inline bool gmm_diag<eT>::em_iterate(const Mat<eT>& X,
    const uword max_iter,
    const eT var_floor,
    const bool verbose)
{
...
template <typename eT>
inline void gmm_diag<eT>::em_update_params(const Mat<eT>& X,
...
template <typename eT>
inline void gmm_diag<eT>::em_generate_acc(const Mat<eT>& X,
