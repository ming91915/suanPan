...
arma_hot arma_inline void arma_set_error(bool& err_state,
    char*& err_msg,
...
arma_inline arma_hot void arma_assert_same_size(const uword A_n_rows,
    const uword A_n_cols,
    const uword B_n_rows,
    const uword B_n_cols,
    const char* x)
...
arma_hot inline void arma_assert_same_size(const uword A_n_rows,
    const uword A_n_cols,
    const uword A_n_slices,
    const uword B_n_rows,
    const uword B_n_cols,
...
inline void arma_assert_cube_as_mat(const Mat<eT>& M,
    const T1& Q,
    const char* x,
...
//! stop if given matrices are incompatible for multiplication
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_mul_size(const Mat<eT1>& A,
    const Mat<eT2>& B,
