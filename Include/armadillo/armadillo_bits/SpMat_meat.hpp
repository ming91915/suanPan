...
template <typename eT> inline SpMat<eT>& SpMat<eT>::operator/=(const SpMat<eT>& x)
{
    ...
    // If you use this method, you are probably stupid or misguided, but for compatibility
    // with Mat, we have implemented it anyway.
    // We have to loop over every element, which is not good.  In fact, it makes me
    // physically sad to write this.
    ...
    const Functor& func)
{
    ...
    if((P.is_alias(*this) == true) ||
        ...
        for(uword i = 0; i < nnz; ++i) {
            t_values[i] = func(x_values[i]); // NOTE: func() must produce a value of type
        ...
        while(it != it_end) {
            access::rw(row_indices[it.pos()]) = it.row();
            access::rw(values[it.pos()]) = func(*it); // NOTE: func() must produce a value
                                                      // of type eT (ie. act as a
                                                      // convertor between eT2 and eT)
            ++access::rw(col_ptrs[it.col() + 1]);
            ++it;
        }
