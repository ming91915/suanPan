...
template <typename eT>
inline Cube<eT>::Cube(const uword in_n_rows,
    ...
    , mat_ptrs(0)
{
    ...
    arma_extra_debug_sigprint(
        arma_str::format("this = %x   in_cube = %x") % this % &in_cube);
...
{
    arma_extra_debug_sigprint(
        arma_str::format("n_rows = %d, n_cols = %d, n_slices = %d") % n_rows % n_cols %
        n_slices);
...
{
    arma_extra_debug_sigprint(
        arma_str::format("in_n_rows = %d, in_n_cols = %d, in_n_slices = %d") % in_n_rows %
    ...
    {
        arma_debug_check((t_mem_state == 2), "Cube::init(): requested size is not "
        ...
        for(uword uslice = 0; uslice < local_n_slices; ++uslice)
            for(uword ucol = 0; ucol < local_n_cols; ++ucol)
                for(uword urow = 0; urow < local_n_rows; ++urow) {
                    *out_mem = std::complex<T>(
                        PX.at(urow, ucol, uslice), PY.at(urow, ucol, uslice));
                    out_mem++;
                }
    }
}
    ...
    if(n_slices == 0) {
        ...
        if(mem_state <= 2) {
            if(n_slices <= Cube_prealloc::mat_ptrs_size) {
                ...
                arma_check_bad_alloc(
                    (mat_ptrs == 0), "Cube::create_mat(): out of memory");
            }
        }
...
//! memory and copying).
template <typename eT>
inline Cube<eT>::Cube(eT* aux_mem,
    ...
    , mat_ptrs(0)
{
    ...
    if(prealloc_mat == true) {
        arma_debug_warn(
            "Cube::Cube(): parameter 'prealloc_mat' ignored as it's no longer used");
    }
...
//! the array is copied.
template <typename eT>
inline Cube<eT>::Cube(const eT* aux_mem,
    const uword aux_n_rows,
    const uword aux_n_cols,
    const uword aux_n_slices)
    : n_rows(aux_n_rows)
    ...
    const uword in_slice2)
{
    ...
    arma_debug_check((in_slice1 > in_slice2) || (in_slice2 >= n_slices),
    ...
    const uword in_slice2)
{
    ...
    arma_debug_check((in_row1 > in_row2) || (in_col1 > in_col2) ||
            (in_slice1 > in_slice2) || (in_row2 >= n_rows) || (in_col2 >= n_cols) ||
            (in_slice2 >= n_slices),
...
//! creation of subview_cube (generic subcube)
template <typename eT>
inline subview_cube<eT> Cube<eT>::subcube(const uword in_row1,
    const uword in_col1,
    const uword in_slice1,
    const SizeCube& s)
{
    ...
    arma_debug_check((in_row1 >= l_n_rows) || (in_col1 >= l_n_cols) ||
            (in_slice1 >= l_n_slices) || ((in_row1 + s_n_rows) > l_n_rows) ||
            ((in_col1 + s_n_cols) > l_n_cols) || ((in_slice1 + s_n_slices) > l_n_slices),
        "Cube::subcube(): indices or size out of bounds");

    return subview_cube<eT>(
        *this, in_row1, in_col1, in_slice1, s_n_rows, s_n_cols, s_n_slices);
}

//! creation of subview_cube (generic subcube)
template <typename eT>
inline const subview_cube<eT> Cube<eT>::subcube(const uword in_row1,
    const uword in_col1,
    const uword in_slice1,
...
template <typename eT>
inline subview_cube<eT> Cube<eT>::
operator()(const span& row_span, const span& col_span, const span& slice_span)
{
    arma_extra_debug_sigprint();
...
template <typename eT>
inline const subview_cube<eT> Cube<eT>::
operator()(const span& row_span, const span& col_span, const span& slice_span) const
{
    arma_extra_debug_sigprint();
...
template <typename eT>
inline subview_cube<eT> Cube<eT>::operator()(const uword in_row1,
    const uword in_col1,
    const uword in_slice1,
    const SizeCube& s)
{
...
arma_inline subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1)
{
    ...
    arma_debug_check(((in_row1 >= n_rows) || (in_col1 >= n_cols)),
    ...
    const uword in_col2)
{
    ...
    return subview_cube<eT>(
        *this, in_row1, in_col1, 0, subcube_n_rows, subcube_n_cols, n_slices);
}
...
Cube<eT>::insert_slices(const uword slice_num, const uword N, const bool set_to_zero)
{
    ...
    arma_debug_check(
        (slice_num > t_n_slices), "Cube::insert_slices(): index out of bounds");
...
//! linear element accessor (treats the cube as a vector); no bounds check.
template <typename eT> arma_inline arma_warn_unused eT& Cube<eT>::at(const uword i)
{
    return access::rw(mem[i]);
}
...
template <typename eT>
arma_inline arma_warn_unused eT& Cube<eT>::
operator()(const uword in_row, const uword in_col, const uword in_slice)
{
    arma_debug_check((in_row >= n_rows) || (in_col >= n_cols) || (in_slice >= n_slices),
...
//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline arma_warn_unused const eT& Cube<eT>::
...
template <typename eT> arma_inline arma_warn_unused bool Cube<eT>::is_finite() const
{
    return arrayops::is_finite(memptr(), n_elem);
}

//! returns true if the cube has no elements
template <typename eT> arma_inline arma_warn_unused bool Cube<eT>::is_empty() const
{
    return (n_elem == 0);
}
...
template <typename eT>
inline arma_warn_unused bool Cube<eT>::in_range(const span& row_span,
    const span& col_span,
    const span& slice_span) const
{
    arma_extra_debug_sigprint();
...
template <typename eT>
inline arma_warn_unused bool Cube<eT>::in_range(const uword in_row,
    const uword in_col,
...
template <typename eT> arma_inline arma_warn_unused eT* Cube<eT>::memptr()
{
    return const_cast<eT*>(mem);
}

//! returns a pointer to array of eTs used by the cube
template <typename eT> arma_inline arma_warn_unused const eT* Cube<eT>::memptr() const
{
    return mem;
}
...
//! returns a pointer to array of eTs used by the specified slice in the cube
template <typename eT>
arma_inline arma_warn_unused eT* Cube<eT>::slice_colptr(const uword uslice,
    const uword col)
{
...
//! returns a pointer to array of eTs used by the specified slice in the cube
template <typename eT>
arma_inline arma_warn_unused const eT* Cube<eT>::slice_colptr(const uword uslice,
...
//! on return, the stream's state are restored to their original values.
template <typename eT>
inline void Cube<eT>::impl_print(std::ostream& user_stream,
...
//! (i.e. the precision and cell width are not modified).
template <typename eT>
inline void Cube<eT>::impl_raw_print(std::ostream& user_stream,
    const std::string& extra_text) const
{
    arma_extra_debug_sigprint();
...
template <typename eT>
arma_deprecated inline void Cube<eT>::reshape(const uword in_rows,
    const uword in_cols,
...
template <typename eT>
inline bool Cube<eT>::save(const std::string name,
...
template <typename eT> inline void Cube<eT>::steal_mem(Cube<eT>& x)
{
    ...
    if((mem_state <= 1) &&
        (((x.mem_state == 0) && (x.n_elem > Cube_prealloc::mem_n_elem)) ||
            (x.mem_state == 1))) {
        reset();
...
arma_inline void Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::mem_setup()
{
    ...
    if(fixed_n_elem > 0) {
        ...
        access::rw(Cube<eT>::mat_ptrs) = const_cast<const Mat<eT>**>(
            (fixed_n_slices > Cube_prealloc::mat_ptrs_size) ? mat_ptrs_local_extra :
                                                              mat_ptrs_local);
...
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
arma_inline arma_warn_unused eT&
Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::at(const uword in_row,
    const uword in_col,
    const uword in_slice)
{
        ...
        for(uword slice = 0; slice < local_n_slices; ++slice)
            for(uword col = 0; col < local_n_cols; ++col)
                for(uword row = 0; row < local_n_rows; ++row) {
                    (*out_mem) =
                        std::complex<T>(P.at(row, col, slice), (*out_mem).imag());
                    out_mem++;
                }
    }
}
    ...
    if(ProxyCube<T1>::use_at == false) {
        ...
        for(uword slice = 0; slice < local_n_slices; ++slice)
            for(uword col = 0; col < local_n_cols; ++col)
                for(uword row = 0; row < local_n_rows; ++row) {
                    (*out_mem) =
                        std::complex<T>((*out_mem).real(), P.at(row, col, slice));
                    out_mem++;
                }
    }
}
