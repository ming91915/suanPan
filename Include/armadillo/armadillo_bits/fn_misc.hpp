    ...
    const uword num = 100u)
{
    ...
    if(num >= 2) {
        ...
        // NOTE: returning "end" for num <= 1 is kept for compatibility with Matlab &
        // Octave,
...
template <typename eT>
arma_warn_unused arma_inline bool is_finite(const eT x,
...
arma_warn_unused inline typename enable_if2<is_arma_type<T1>::value, bool>::result
...
arma_warn_unused inline uvec ind2sub(const SizeCube& s, const uword i)
{
    ...
    arma_debug_check(
        (i >= (s_n_elem_slice * s.n_slices)), "ind2sub(): index out of range");
...
arma_warn_unused arma_inline uword sub2ind(const SizeMat& s,
    const uword row,
    const uword col)
{
    arma_extra_debug_sigprint();
    ...
    arma_debug_check(
        ((row >= s_n_rows) || (col >= s.n_cols)), "sub2ind(): subscript out of range");
...
arma_warn_unused inline uvec sub2ind(const SizeMat& s, const Base<uword, T1>& subscripts)
{
    ...
    arma_debug_check(
        (U.M.n_rows != 2), "sub2ind(): matrix of subscripts must have 2 rows");
...
arma_warn_unused arma_inline uword sub2ind(const SizeCube& s,
    const uword row,
    const uword col,
    const uword slice)
{
...
arma_warn_unused inline uvec sub2ind(const SizeCube& s, const Base<uword, T1>& subscripts)
{
    ...
    arma_debug_check(
        (U.M.n_rows != 3), "sub2ind(): matrix of subscripts must have 3 rows");
