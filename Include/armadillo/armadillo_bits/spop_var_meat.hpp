    ...
    if(dim == 0) // find variance in each column
    {
        ...
        for(uword col = 0; col < p_n_cols; ++col) {
            if(SpProxy<T1>::use_iterator) {
                ...
                out.at(0, col) =
                    spop_var::direct_var(&p.get_values()[p.get_col_ptrs()[col]],
...
inline eT spop_var::direct_var(const eT* const X,
    const uword length,
    const uword N,
    const uword norm_type)
{
...
inline eT spop_var::iterator_var(T1& it,
    const T1& end,
    const uword n_zero,
    const uword norm_type,
    const eT junk1,
...
template <typename T1, typename eT>
inline typename get_pod_type<eT>::result spop_var::iterator_var(T1& it,
    const T1& end,
    const uword n_zero,
    const uword norm_type,
    const eT junk1,
