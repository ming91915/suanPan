...
arma_warn_unused arma_inline const Op<T1, op_prod> prod(const T1& X,
    const uword dim = 0,
...
template <typename T1>
arma_warn_unused inline typename T1::elem_type prod(const T1& X,
...
template <typename T1>
arma_warn_unused inline const Op<Op<T1, op_prod>, op_prod> prod(const Op<T1, op_prod>& in,
    const uword dim)
{
    arma_extra_debug_sigprint();
