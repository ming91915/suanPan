...
template <typename eT> inline void subview_cube<eT>::operator=(const eT val)
{
    ...
    if(n_elem != 1) {
        arma_debug_assert_same_size(
            n_rows, n_cols, n_slices, 1, 1, 1, "copy into subcube");
    }
...
inline void subview_cube<eT>::operator=(const BaseCube<eT, T1>& in)
{
    ...
    for(uword slice = 0; slice < t_n_slices; ++slice) {
        for(uword col = 0; col < t_n_cols; ++col) {
            arrayops::copy(
                t.slice_colptr(slice, col), x.slice_colptr(slice, col), t_n_rows);
        }
    }
}
...
inline void subview_cube<eT>::operator=(const GenCube<eT, gen_type>& in)
{
    ...
    arma_debug_assert_same_size(
        n_rows, n_cols, n_slices, in.n_rows, in.n_cols, in.n_slices, "copy into subcube");
...
template <typename eT>
arma_inline eT& subview_cube<eT>::
...
inline void subview_cube<eT>::extract(Cube<eT>& out, const subview_cube<eT>& in)
{
    ...
    arma_extra_debug_print(
        arma_str::format("out.n_rows = %d   out.n_cols = %d    out.n_slices = %d    "
    ...
    for(uword slice = 0; slice < n_slices; ++slice) {
        for(uword col = 0; col < n_cols; ++col) {
            arrayops::copy(
                out.slice_colptr(slice, col), in.slice_colptr(slice, col), n_rows);
        }
    }
}
