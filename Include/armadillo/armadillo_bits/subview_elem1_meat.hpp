    ...
    , a(in_a)
{
    ...
    // TODO: refactor to unwrap 'in_a' instead of storing a ref to it; this will allow
    // removal of carrying T1 around and repetition of size checks
}
...
inline void subview_elem1<eT, T1>::inplace_op(const eT val)
{
    ...
    for(iq = 0, jq = 1; jq < aa_n_elem; iq += 2, jq += 2) {
        ...
        arma_debug_check(
            ((ii >= m_n_elem) || (jj >= m_n_elem)), "Mat::elem(): index out of bounds");
...
inline void subview_elem1<eT, T1>::inplace_op(const subview_elem1<eT, T2>& x)
{
    ...
    if(&(s.m) == &(x.m)) {
        ...
        for(iq = 0, jq = 1; jq < s_aa_n_elem; iq += 2, jq += 2) {
            ...
            arma_debug_check((s_ii >= s_m_n_elem) || (s_jj >= s_m_n_elem) ||
        ...
        if(iq < s_aa_n_elem) {
            ...
            arma_debug_check(((s_ii >= s_m_n_elem) || (x_ii >= x_m_n_elem)),
                "Mat::elem(): index out of bounds");
...
inline void subview_elem1<eT, T1>::inplace_op(const Base<eT, T2>& x)
{
    ...
    if((is_alias == false) && (Proxy<T2>::use_at == false)) {
        ...
        for(iq = 0, jq = 1; jq < aa_n_elem; iq += 2, jq += 2) {
            ...
            arma_debug_check(((ii >= m_n_elem) || (jj >= m_n_elem)),
                "Mat::elem(): index out of bounds");
