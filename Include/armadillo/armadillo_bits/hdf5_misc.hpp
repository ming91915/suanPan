...
template <> inline hid_t get_hdf5_type<std::complex<float>>()
{
    ...
    arma_H5Tinsert(
        type, "real", HOFFSET(hdf5_complex_t<float>, real), arma_H5T_NATIVE_FLOAT);
    arma_H5Tinsert(
        type, "imag", HOFFSET(hdf5_complex_t<float>, imag), arma_H5T_NATIVE_FLOAT);
...
template <> inline hid_t get_hdf5_type<std::complex<double>>()
{
    ...
    arma_H5Tinsert(
        type, "real", HOFFSET(hdf5_complex_t<double>, real), arma_H5T_NATIVE_DOUBLE);
    arma_H5Tinsert(
        type, "imag", HOFFSET(hdf5_complex_t<double>, imag), arma_H5T_NATIVE_DOUBLE);
    ...
    )
{
    ...
    if(info->type == H5O_TYPE_DATASET) {
        ...
        for(size_t string_pos = 0; string_pos < search_info->best_match_position;
            ...
            // If they are not the same, but we have not found anything and we don't need
            // an exact match, take this.
...
//! it will be considered -- for instance, a one-dimensional HDF5 vector can be loaded as
