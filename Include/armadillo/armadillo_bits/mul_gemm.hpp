...
public:
    ...
    arma_hot inline static void apply(Mat<eT>& C,
        const TA& A,
        const TB& B,
        const eT alpha = eT(1),
        const eT beta = eT(0))
...
public:
        ...
        const eT beta = eT(0))
    {
        ...
        if((do_trans_A == false) && (do_trans_B == false)) {
            ...
            gemm_emul_large<false, false, use_alpha, use_beta>::apply(
                C, A, BB, alpha, beta);
...
//! \brief
//! Wrapper for ATLAS/BLAS dgemm function, using template arguments to control the
//! arguments passed to dgemm.
...
public:
        ...
        const eT beta = eT(0))
    {
        ...
        if((A.n_rows <= 4) && (A.n_rows == A.n_cols) && (A.n_rows == B.n_rows) &&
            ...
            if(do_trans_B == false) {
                gemm_emul_tinysq<do_trans_A, use_alpha, use_beta>::apply(
                    C, A, B, alpha, beta);
            } else {
...
#if defined(ARMA_USE_ATLAS)
            {
                ...
                atlas::cblas_gemm<eT>(atlas::CblasColMajor,
                    (do_trans_A) ? (is_cx<eT>::yes ? CblasConjTrans : atlas::CblasTrans) :
                                   atlas::CblasNoTrans,
                    (do_trans_B) ? (is_cx<eT>::yes ? CblasConjTrans : atlas::CblasTrans) :
                                   atlas::CblasNoTrans,
...
#elif defined(ARMA_USE_BLAS)
            {
                ...
                arma_extra_debug_print(
                    arma_str::format("blas::gemm(): trans_A = %c") % trans_A);
                arma_extra_debug_print(
                    arma_str::format("blas::gemm(): trans_B = %c") % trans_B);

                blas::gemm<eT>(&trans_A, &trans_B, &m, &n, &k, &local_alpha, A.mem, &lda,
                    B.mem, &ldb, &local_beta, C.memptr(), &m);
            }
#else
            {
