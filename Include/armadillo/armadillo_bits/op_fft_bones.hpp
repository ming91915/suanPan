...
public:
    ...
    copy_vec(typename Proxy<T1>::elem_type* dest, const Proxy<T1>& P, const uword N);
    template <typename T1>
    arma_hot inline static void copy_vec_proxy(typename Proxy<T1>::elem_type* dest,
        const Proxy<T1>& P,
        const uword N);
    template <typename T1>
    arma_hot inline static void copy_vec_unwrap(typename Proxy<T1>::elem_type* dest,
        const Proxy<T1>& P,
        const uword N);
};
