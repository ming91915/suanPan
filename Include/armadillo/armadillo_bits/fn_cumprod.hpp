...
arma_warn_unused arma_inline
    typename enable_if2<is_arma_type<T1>::value, const Op<T1, op_cumprod_default>>::result
    cumprod(const T1& X)
{
...
template <typename T>
arma_warn_unused arma_inline const typename arma_scalar_only<T>::result& cumprod(
    const T& x)
{
    return x;
}
