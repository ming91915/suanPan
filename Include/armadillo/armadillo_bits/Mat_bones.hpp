...
public:
    ...
    inline Mat(const uword in_rows,
        const uword in_cols,
    ...
    inline explicit Mat(const subview<eT>& X,
    ...
    arma_inline subview<eT> submat(const uword in_row1,
        const uword in_col1,
        const uword in_row2,
        const uword in_col2);
    arma_inline const subview<eT> submat(const uword in_row1,
        const uword in_col1,
        const uword in_row2,
    ...
    inline void
    insert_cols(const uword col_num, const uword N, const bool set_to_zero = true);
    ...
    public:
        ...
        inline const_row_col_iterator(const Mat<eT>& in_M,
            const uword row = 0,
            const uword col = 0);
...
protected:
    ...
    inline Mat(const char junk,
        const eT* aux_mem,
        const uword aux_n_rows,
        const uword aux_n_cols);

    inline Mat(const arma_vec_indicator&, const uhword in_vec_state);
    inline Mat(const arma_vec_indicator&,
        const uword in_n_rows,
        const uword in_n_cols,
        const uhword in_vec_state);

    inline Mat(const arma_fixed_indicator&,
        const uword in_n_rows,
        const uword in_n_cols,
        const uhword in_vec_state,
        const eT* in_mem);
