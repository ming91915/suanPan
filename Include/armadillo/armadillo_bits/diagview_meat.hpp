...
template <typename eT> inline void diagview<eT>::operator=(const diagview<eT>& x)
{
    ...
    arma_debug_check(
        (d.n_elem != x.n_elem), "diagview: diagonals have incompatible lengths");
...
inline void diagview<eT>::minus_inplace(Mat<eT>& out, const diagview<eT>& in)
{
    ...
    arma_debug_assert_same_size(
        out.n_rows, out.n_cols, in.n_rows, in.n_cols, "subtraction");
...
inline void diagview<eT>::schur_inplace(Mat<eT>& out, const diagview<eT>& in)
{
    ...
    arma_debug_assert_same_size(
        out.n_rows, out.n_cols, in.n_rows, in.n_cols, "element-wise multiplication");
...
inline void diagview<eT>::div_inplace(Mat<eT>& out, const diagview<eT>& in)
{
    ...
    arma_debug_assert_same_size(
        out.n_rows, out.n_cols, in.n_rows, in.n_cols, "element-wise division");
...
template <typename eT> arma_inline eT diagview<eT>::at_alt(const uword ii) const
{
...
template <typename eT> arma_inline eT& diagview<eT>::at(const uword row, const uword)
{
    return (const_cast<Mat<eT>&>(m)).at(row + row_offset, row + col_offset);
}

template <typename eT> arma_inline eT diagview<eT>::at(const uword row, const uword) const
{
...
{
    arma_debug_check(
        ((row >= n_elem) || (col > 0)), "diagview::operator(): out of bounds");
