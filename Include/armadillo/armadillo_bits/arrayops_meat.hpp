...
arma_hot inline void arrayops::convert(out_eT* dest, const in_eT* src, const uword n_elem)
{
    ...
    for(j = 1; j < n_elem; j += 2) {
        ...
        (*dest) = (is_signed<out_eT>::value) ?
            out_eT(tmp_i) :
            (cond_rel<is_signed<in_eT>::value>::lt(tmp_i, in_eT(0)) ? out_eT(0) :
                                                                      out_eT(tmp_i));
        ...
        (*dest) = (is_signed<out_eT>::value) ?
            out_eT(tmp_j) :
            (cond_rel<is_signed<in_eT>::value>::lt(tmp_j, in_eT(0)) ? out_eT(0) :
                                                                      out_eT(tmp_j));
        dest++;
    }
...
arma_hot inline void arrayops::inplace_set(eT* dest, const eT val, const uword n_elem)
{
    ...
    if((n_elem <= 9) && (is_cx<eT>::no)) {
        ...
        if((val == eT(0)) &&
            (std::numeric_limits<eT>::is_integer ||
                (std::numeric_limits<pod_type>::is_iec559 && is_real<pod_type>::value))) {
