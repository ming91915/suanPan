...
inline void glue_cor::direct_cor(Mat<eT>& out,
    const Mat<eT>& A,
    const Mat<eT>& B,
    const uword norm_type)
{
    ...
    if(A.is_vec() && B.is_vec()) {
        ...
        const Mat<eT> stddev_A =
            (A.n_rows == 1) ? Mat<eT>(stddev(trans(A))) : Mat<eT>(stddev(A));
        const Mat<eT> stddev_B =
            (B.n_rows == 1) ? Mat<eT>(stddev(trans(B))) : Mat<eT>(stddev(B));
    ...
    const uword norm_type)
{
    ...
    if(A.is_vec() && B.is_vec()) {
        ...
        const Mat<T> stddev_A =
            (A.n_rows == 1) ? Mat<T>(stddev(trans(A))) : Mat<T>(stddev(A));
