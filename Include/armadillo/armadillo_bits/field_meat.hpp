...
//! construct the field with the specified dimensions
template <typename oT>
inline field<oT>::field(const uword n_rows_in,
...
{
    arma_extra_debug_sigprint(
        arma_str::format("n_rows_in = %d, n_cols_in = %d") % n_rows_in % n_cols_in);
...
{
    arma_extra_debug_sigprint(
        arma_str::format("n_rows_in = %d, n_cols_in = %d, n_slices_in = %d") % n_rows_in %
        n_cols_in % n_slices_in);

    init(n_rows_in, n_cols_in, n_slices_in);
}

template <typename oT> inline void field<oT>::set_size(const SizeMat& s)
{
    init(s.n_rows, s.n_cols);
}

template <typename oT> inline void field<oT>::set_size(const SizeCube& s)
{
    init(s.n_rows, s.n_cols, s.n_slices);
}
    ...
    for(; it != it_end; ++it) {
        if(x_n_cols_found == false) {
            ...
            arma_check((uword((*it).size()) != x_n_cols),
                "field::init(): inconsistent number of columns in initialiser list");
        }
    }
...
//! linear element accessor (treats the field as a vector); no bounds check
template <typename oT> arma_inline const oT& field<oT>::at(const uword i) const
{
    return (*mem[i]);
}
...
{
    arma_debug_check(((in_row >= n_rows) || (in_col >= n_cols) || (0 >= n_slices)),
    ...
    const uword in_slice2)
{
    ...
    return subview_field<oT>(
        *this, in_row1, in_col1, in_slice1, sub_n_rows, sub_n_cols, sub_n_slices);
}
    ...
    const SizeCube& s)
{
    ...
    return subview_field<oT>(
        *this, in_row1, in_col1, in_slice1, s_n_rows, s_n_cols, sub_n_slices);
}
...
template <typename oT>
inline subview_field<oT> field<oT>::
...
template <typename oT>
inline const subview_field<oT> field<oT>::
...
template <typename oT> inline void field<oT>::print(const std::string extra_text) const
{
    arma_extra_debug_sigprint();
...
template <typename oT> inline const oT& field<oT>::const_iterator::operator*() const
{
    return M[i];
}
...
template <typename oT>
inline bool field_aux::save(const field<oT>&,
    const std::string&,
    const file_type,
    std::string& err_msg)
{
    ...
    err_msg =
        " [saving/loading this type of field is currently not supported] filename = ";
...
inline bool field_aux::load(field<std::string>& x,
    const std::string& name,
    const file_type type,
    std::string& err_msg)
{
