...
arma_inline SpSubview<eT>::SpSubview(const SpMat<eT>& in_m,
    const uword in_row1,
    const uword in_col1,
...
template <typename eT>
arma_inline SpSubview<eT>::SpSubview(SpMat<eT>& in_m,
    const uword in_row1,
    const uword in_col1,
    const uword in_n_rows,
    const uword in_n_cols)
...
inline const SpSubview<eT>& SpSubview<eT>::operator=(const Base<eT, T1>& in)
{
    ...
    arma_debug_assert_same_size(
        n_rows, n_cols, b.n_rows, b.n_cols, "insertion into sparse submatrix");
    ...
    while(x_it_ok || y_it_ok) {
        const bool x_inside_box = (x_it_row >= pa_start_row) &&
            (x_it_row <= pa_end_row) && (x_it_col >= pa_start_col) &&
            (x_it_col <= pa_end_col);
        const bool y_inside_box = (y_it_row >= pa_start_row) &&
            (y_it_row <= pa_end_row) && (y_it_col >= pa_start_col) &&
            (y_it_col <= pa_end_col);
...
template <typename eT>
inline SpSubview<eT> SpSubview<eT>::submat(const uword in_row1,
    const uword in_col1,
    const uword in_row2,
    const uword in_col2)
{
    ...
    return access::rw(m).submat(
        in_row1 + aux_row1, in_col1 + aux_col1, in_row2 + aux_row1, in_col2 + aux_col1);
}

template <typename eT>
inline const SpSubview<eT> SpSubview<eT>::submat(const uword in_row1,
    const uword in_col1,
    const uword in_row2,
    const uword in_col2) const
{
...
inline SpSubview<eT> SpSubview<eT>::submat(const span& row_span, const span& col_span)
{
    ...
    arma_debug_check((row_all ? false : ((in_row1 > in_row2) || (in_row2 >= n_rows))) ||
...
template <typename eT> inline typename SpSubview<eT>::iterator SpSubview<eT>::begin()
{
