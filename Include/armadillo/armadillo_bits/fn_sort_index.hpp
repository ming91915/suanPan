...
sort_index(const Base<typename T1::elem_type, T1>& X, const uword sort_type)
{
    arma_extra_debug_sigprint();
    ...
    arma_debug_check(
        (sort_type > 1), "sort_index(): parameter 'sort_type' must be 0 or 1");
...
sort_index(const T1& X, const T2* sort_direction)
{
    ...
    arma_debug_check(
        ((sig != 'a') && (sig != 'd')), "sort_index(): unknown sort direction");

    return mtOp<uword, T1, op_sort_index>(
...
stable_sort_index(const Base<typename T1::elem_type, T1>& X, const uword sort_type)
{
    arma_extra_debug_sigprint();

    // arma_debug_warn("stable_sort_index(X,uword) is deprecated and will be removed;
    // change to stable_sort_index(X,sort_direction)");

    arma_debug_check(
        (sort_type > 1), "stable_sort_index(): parameter 'sort_type' must be 0 or 1");
...
stable_sort_index(const T1& X, const T2* sort_direction)
{
    ...
    arma_debug_check(
        ((sig != 'a') && (sig != 'd')), "stable_sort_index(): unknown sort direction");
