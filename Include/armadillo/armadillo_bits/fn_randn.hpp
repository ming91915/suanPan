...
arma_warn_unused arma_inline const Gen<obj_type, gen_randn> randn(const uword n_elem,
...
//! Generate a dense matrix with all elements set to random values with a gaussian
//! distribution (zero mean, unit variance)
arma_warn_unused arma_inline const Gen<mat, gen_randn> randn(const uword n_rows,
    const uword n_cols)
{
    arma_extra_debug_sigprint();
...
template <typename obj_type>
arma_warn_unused arma_inline const Gen<obj_type, gen_randn> randn(const uword n_rows,
    const uword n_cols,
...
template <typename obj_type>
arma_warn_unused arma_inline const Gen<obj_type, gen_randn> randn(const SizeMat& s,
    const typename arma_Mat_Col_Row_only<obj_type>::result* junk = 0)
{
    arma_extra_debug_sigprint();
    arma_ignore(junk);
...
arma_warn_unused arma_inline const GenCube<cube::elem_type, gen_randn>
...
arma_warn_unused arma_inline const GenCube<cube::elem_type, gen_randn> randn(
    const SizeCube& s)
{
    arma_extra_debug_sigprint();
...
arma_warn_unused arma_inline const GenCube<typename cube_type::elem_type, gen_randn>
randn(const uword n_rows,
    const uword n_cols,
...
randn(const SizeCube& s, const typename arma_Cube_only<cube_type>::result* junk = 0)
{
    ...
    return GenCube<typename cube_type::elem_type, gen_randn>(
