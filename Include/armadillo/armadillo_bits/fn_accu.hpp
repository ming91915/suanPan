...
arma_hot inline typename T1::elem_type accu_proxy_linear(const Proxy<T1>& P)
{
    ...
    if(arma_config::openmp && Proxy<T1>::use_mp && mp_gate<eT>::eval(n_elem)) {
        ...
        {
            // NOTE: using parallelisation with manual reduction workaround to take into
...
template <typename eT> arma_warn_unused arma_hot inline eT accu(const subview<eT>& X)
{
    ...
    if(X_n_rows == 1) {
        ...
        const sv_type& sv = reinterpret_cast<const sv_type&>(
            X); // subview_row<eT> is a child class of subview<eT> and has no extra data
