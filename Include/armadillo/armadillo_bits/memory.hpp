...
arma_inline uword memory::enlarge_to_mult_of_chunksize(const uword n_elem)
{
    ...
    const uword n_elem_mod =
        (n_elem > 0) ? (((n_elem - 1) / chunksize) + 1) * chunksize : uword(0);
...
template <typename eT> inline arma_malloc eT* memory::acquire(const uword n_elem)
{
...
#elif defined(ARMA_HAVE_POSIX_MEMALIGN)
    {
        ...
        const size_t alignment =
            16; // change the 16 to 64 if you wish to align to the cache line

        int status = posix_memalign((void**)&memptr,
    ...
    if(n_elem > 0) {
        arma_check_bad_alloc(
            (out_memptr == NULL), "arma::memory::acquire(): out of memory");
    }
