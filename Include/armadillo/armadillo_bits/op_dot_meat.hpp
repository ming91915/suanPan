...
arma_hot arma_inline typename arma_not_cx<eT>::result
...
template <typename eT>
arma_hot inline typename arma_cx_only<eT>::result
op_dot::direct_dot_arma(const uword n_elem, const eT* const A, const eT* const B)
{
    arma_extra_debug_sigprint();
...
template <typename eT>
arma_hot inline typename arma_real_only<eT>::result
...
template <typename eT>
inline arma_hot typename arma_cx_only<eT>::result
op_dot::direct_dot(const uword n_elem, const eT* const A, const eT* const B)
{
    if(n_elem <= 16u) {
        return op_dot::direct_dot_arma(n_elem, A, B);
...
template <typename eT>
arma_hot inline typename arma_integral_only<eT>::result
op_dot::direct_dot(const uword n_elem, const eT* const A, const eT* const B)
{
    return op_dot::direct_dot_arma(n_elem, A, B);
}
...
arma_hot inline typename T1::elem_type op_dot::apply(const T1& X, const T2& Y)
{
    ...
    const bool have_direct_mem = ((is_Mat<T1>::value || is_subview_col<T1>::value) &&
        (is_Mat<T2>::value || is_subview_col<T2>::value));
    ...
    if(use_at || have_direct_mem) {
        ...
        if(is_subview_row<T1>::value && is_subview_row<T2>::value) {
            ...
            if((A.m.n_rows == 1) && (B.m.n_rows == 1)) {
                ...
                return op_dot::direct_dot(
                    A.n_elem, &A_mem[A.aux_col1], &B_mem[B.aux_col1]);
            }
        }
...
template <typename T1, typename T2>
arma_hot inline typename arma_not_cx<typename T1::elem_type>::result
...
template <typename T1, typename T2>
arma_hot inline typename arma_cx_only<typename T1::elem_type>::result
op_dot::apply_proxy(const Proxy<T1>& PA, const Proxy<T2>& PB)
{
    arma_extra_debug_sigprint();
...
op_cdot::direct_cdot(const uword n_elem, const eT* const A, const eT* const B)
{
    ...
    if(n_elem <= 32u) {
        ...
        {
            // TODO: use dedicated atlas functions cblas_cdotc_sub() and cblas_zdotc_sub()
            // and retune threshold
...
arma_hot inline typename T1::elem_type op_cdot::apply_unwrap(const T1& X, const T2& Y)
{
    ...
    arma_debug_check(
        (A.n_elem != B.n_elem), "cdot(): objects must have the same number of elements");
    ...
    op_dot_mixed::apply(const T1& A, const T2& B)
{
    ...
    arma_debug_check(
        (N != PB.get_n_elem()), "dot(): objects must have the same number of elements");
