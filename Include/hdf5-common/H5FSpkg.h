...
#define H5FS_HEADER_SIZE(f)                                                              \
    (/* General metadata fields */                                                       \
        ...
        + (unsigned)H5F_SIZEOF_SIZE(                                                     \
              f) /* Allocated size of serialized free space sections */                  \
        )
...
#define H5FS_SINFO_PREFIX_SIZE(f)                                      \
    (/* General metadata fields */                                     \
        ...
        + (unsigned)H5F_SIZEOF_ADDR(                                   \
              f) /* Address of free space header for these sections */ \
        )
...
typedef struct H5FS_sinfo_t {
    ...
    size_t
        serial_size_count; /* Total number of differently sized serializable sections */
    size_t
        ghost_size_count; /* Total number of differently sized un-serializable sections */
...
struct H5FS_t {
    ...
    unsigned shrink_percent; /* Percent of "normal" serialized size to shrink serialized
                                space at */
    unsigned expand_percent; /* Percent of "normal" serialized size to expand serialized
                                space at */
    unsigned max_sect_addr;  /* Size of address space free sections are within (log2 of
                                actual value) */
...
H5_DLL int H5FS_cmp_cparam_test(const H5FS_create_t* cparam1,
