...
typedef enum H5D_chunk_index_t {
    ...
    H5D_CHUNK_IDX_SINGLE = 1, /* Single Chunk index (cur dims[]=max dims[]=chunk dims[];
                                 filtered & non-filtered) */
    H5D_CHUNK_IDX_NONE =
        2, /* Implicit: No Index (H5D_ALLOC_TIME_EARLY, non-filtered, fixed dims) */
...
typedef herr_t (*H5D_operator_t)(void* elem,
    hid_t type_id,
    unsigned ndim,
    const hsize_t* point,
    void* operator_data);
...
typedef herr_t (
    *H5D_gather_func_t)(const void* dst_buf, size_t dst_buf_bytes_used, void* op_data);
...
H5_DLL herr_t H5Dvlen_get_buf_size(hid_t dataset_id,
    ...
    hsize_t* size);
H5_DLL herr_t
H5Dfill(const void* fill, hid_t fill_type, void* buf, hid_t buf_type, hid_t space);
...
/* Function prototypes */
H5_DLL hid_t
H5Dcreate1(hid_t file_id, const char* name, hid_t type_id, hid_t space_id, hid_t dcpl_id);
