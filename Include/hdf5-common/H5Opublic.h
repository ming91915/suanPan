...
#define H5O_COPY_EXPAND_SOFT_LINK_FLAG                \
    (0x0002u) /* Expand soft links into new objects \ \
                 */
#define H5O_COPY_EXPAND_EXT_LINK_FLAG \
    (0x0004u) /* Expand external links into new objects */
#define H5O_COPY_EXPAND_REFERENCE_FLAG \
    (0x0008u) /* Copy objects that are pointed by references */
...
#define H5O_COPY_MERGE_COMMITTED_DTYPE_FLAG \
    (0x0040u)                  /* Merge committed datatypes in dest file */
...
#define H5O_HDR_STORE_TIMES \
    0x20 /* Store access, modification, change & birth times for object */
#define H5O_HDR_ALL_FLAGS                                                  \
    (H5O_HDR_CHUNK0_SIZE | H5O_HDR_ATTR_CRT_ORDER_TRACKED |                \
        H5O_HDR_ATTR_CRT_ORDER_INDEXED | H5O_HDR_ATTR_STORE_PHASE_CHANGE | \
        H5O_HDR_STORE_TIMES)
...
typedef struct H5O_info_t {
    ...
    struct {
        H5_ih_info_t obj;  /* v1/v2 B-tree & local/fractal heap for groups, B-tree for
                              chunked datasets */
...
typedef herr_t (
    *H5O_iterate_t)(hid_t obj, const char* name, const H5O_info_t* info, void* op_data);

typedef enum H5O_mcdt_search_ret_t {
    H5O_MCDT_SEARCH_ERROR = -1, /* Abort H5Ocopy */
    H5O_MCDT_SEARCH_CONT, /* Continue the global search of all committed datatypes in the
                             destination file */
    H5O_MCDT_SEARCH_STOP /* Stop the search, but continue copying.  The committed datatype
                            will be copied but not merged. */
} H5O_mcdt_search_ret_t;

/* Callback to invoke when completing the search for a matching committed datatype from
 * the committed dtype list */
