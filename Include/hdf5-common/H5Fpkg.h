...
#define H5F_SUPER_ALL_FLAGS \
    (H5F_SUPER_WRITE_ACCESS | H5F_SUPER_FILE_OK | H5F_SUPER_SWMR_WRITE_ACCESS)
...
#define H5F_FS_MERGE_RAWDATA                                     \
    0x02 /* Section can merge with small 'raw' data aggregator \ \
            */
...
#define H5F_MAX_DRVINFOBLOCK_SIZE                                                       \
    1024                             /* Maximum size of superblock driver info buffer \ \
                                        */
...
/* For superblock version 0 & 1:
   Offset to the file consistency flags (status_flags) in the superblock (excluding
   H5F_SUPERBLOCK_FIXED_SIZE) */
...
#define H5F_SUPER_STATUS_FLAGS_OFF(v) \
    (H5F_SUPERBLOCK_FIXED_SIZE + H5F_SUPER_STATUS_OFF(v))
...
struct H5F_file_t {
    ...
    hbool_t evict_on_close; /* If the file's objects should be evicted from the metadata
                               cache on close */
    ...
    hbool_t point_of_no_return; /* flag to indicate that we can't go back and delete a
                                   freespace header when it's used up */

    H5F_fs_state_t
        fs_state[H5F_MEM_PAGE_NTYPES];    /* State of free space manager for each type */
    haddr_t fs_addr[H5F_MEM_PAGE_NTYPES]; /* Address of free space manager info for each
                                             type */
    ...
    unsigned fs_aggr_merge[H5FD_MEM_NTYPES]; /* Flags for whether free space can merge
                                                with aggregator(s) */
    H5FD_mem_t fs_type_map[H5FD_MEM_NTYPES]; /* Mapping of "real" file space type into
                                                tracked type */
    H5F_blk_aggr_t
        meta_aggr; /* Metadata aggregation info (if aggregating metadata allocations) */
    H5F_blk_aggr_t sdata_aggr; /* "Small data" aggregation info (if aggregating "small
                                  data" allocations) */
    ...
    /* Metadata retry info */
    unsigned
        read_attempts; /* The # of reads to try when reading metadata with checksum */
    unsigned retries_nbins;         /* # of bins for each retries[] */
    uint32_t* retries[H5AC_NTYPES]; /* Track # of read retries for metdata items with
                                       checksum */
...
H5_DLL ssize_t H5F_get_file_image(H5F_t* f,
    void* buf_ptr,
    size_t buf_len,
    hid_t meta_dxpl_id,
    hid_t raw_dxpl_id);
...
H5_DLL herr_t H5F__accum_read(const H5F_io_info2_t* fio_info,
    H5FD_mem_t type,
    haddr_t addr,
    size_t size,
    void* buf);
H5_DLL herr_t H5F__accum_write(const H5F_io_info2_t* fio_info,
    H5FD_mem_t type,
    haddr_t addr,
    size_t size,
    const void* buf);
H5_DLL herr_t H5F__accum_free(const H5F_io_info2_t* fio_info,
    H5FD_mem_t type,
    haddr_t addr,
    hsize_t size);
...
H5_DLL haddr_t H5F_alloc(H5F_t* f,
    hid_t dxpl_id,
    H5F_mem_t type,
    hsize_t size,
    haddr_t* frag_addr,
    hsize_t* frag_size);
H5_DLL herr_t
H5F_free(H5F_t* f, hid_t dxpl_id, H5F_mem_t type, haddr_t addr, hsize_t size);
H5_DLL htri_t H5F_try_extend(H5F_t* f,
    hid_t dxpl_id,
    H5FD_mem_t type,
    haddr_t blk_end,
...
/* Testing functions */
#ifdef H5F_TESTING
H5_DLL herr_t H5F_get_sohm_mesg_count_test(hid_t fid,
    unsigned type_id,
    size_t* mesg_count);
