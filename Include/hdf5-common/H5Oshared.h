...
#error "Need to define H5O_SHARED_DECODE_REAL macro!"
    ...
    if(mesg_flags & H5O_MSG_FLAG_SHARED) {
        ...
        if(NULL == (ret_value = H5O_shared_decode(
                        f, dxpl_id, open_oh, ioflags, p, H5O_SHARED_TYPE)))
...
/* We currently do not support automatically fixing shared messages */
        ...
        if(*ioflags & H5O_DECODEIO_DIRTY)
            HGOTO_ERROR(
                H5E_OHDR, H5E_UNSUPPORTED, NULL, "unable to mark shared message dirty")
        ...
        *ioflags &= ~H5O_DECODEIO_DIRTY;
        ...
        if(NULL == (ret_value = H5O_SHARED_DECODE_REAL(
                        f, dxpl_id, open_oh, mesg_flags, ioflags, p)))
...
static H5_INLINE herr_t H5O_SHARED_ENCODE(H5F_t* f,
    ...
    const void* _mesg)
{
    const H5O_shared_t* sh_mesg = (const H5O_shared_t*)
        _mesg;                  /* Pointer to shared message portion of actual message */
...
#error "Need to define H5O_SHARED_ENCODE_REAL macro!"
    ...
    /* Sanity check */
    HDassert(sh_mesg->type == H5O_SHARE_TYPE_UNSHARED ||
        sh_mesg->msg_type_id == H5O_SHARED_TYPE->id);
...
#error "Need to define H5O_SHARED_SIZE_REAL macro!"
    ...
    if(H5O_IS_STORED_SHARED(sh_mesg->type) && !disable_shared) {
        ...
        if(0 == (ret_value = H5O_shared_size(f, sh_mesg)))
            HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, 0,
                "unable to retrieve encoded size of shared message")
    } /* end if */
    else {
        /* Retrieve size of native message directly */
        if(0 == (ret_value = H5O_SHARED_SIZE_REAL(f, _mesg)))
            HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, 0,
                "unable to retrieve encoded size of native message")
    } /* end else */
    ...
    void* _mesg)
{
    H5O_shared_t* sh_mesg =
        (H5O_shared_t*)_mesg;   /* Pointer to shared message portion of actual message */
    herr_t ret_value = SUCCEED; /* Return value */
...
#error "Need to define H5O_SHARED_DELETE macro!"
    ...
    if(H5O_IS_TRACKED_SHARED(sh_mesg->type)) {
        ...
        if(H5O_shared_delete(f, dxpl_id, open_oh, H5O_SHARED_TYPE, sh_mesg) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTDEC, FAIL,
                "unable to decrement ref count for shared message")
    } /* end if */
        ...
        if(H5O_SHARED_DELETE_REAL(f, dxpl_id, open_oh, _mesg) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTDEC, FAIL,
                "unable to decrement ref count for native message")
    }  /* end else */
...
#error "Need to define H5O_SHARED_LINK macro!"
    ...
    if(H5O_IS_TRACKED_SHARED(sh_mesg->type)) {
        ...
        if(H5O_shared_link(f, dxpl_id, open_oh, H5O_SHARED_TYPE, sh_mesg) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTINC, FAIL,
                "unable to increment ref count for shared message")
    } /* end if */
        ...
        if(H5O_SHARED_LINK_REAL(f, dxpl_id, open_oh, _mesg) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTINC, FAIL,
                "unable to increment ref count for native message")
    }  /* end else */
...
#error "Need to define H5O_SHARED_COPY_FILE macro!"
    ...
    if(NULL == (dst_mesg = H5O_SHARED_COPY_FILE_REAL(file_src, H5O_SHARED_TYPE,
                    _native_src, file_dst, recompute_size, cpy_info, udata, dxpl_id)))
        HGOTO_ERROR(
            H5E_OHDR, H5E_CANTCOPY, NULL, "unable to copy native message to another file")
    ...
    if(H5O_shared_copy_file(file_src, file_dst, H5O_SHARED_TYPE, _native_src, dst_mesg,
           recompute_size, mesg_flags, cpy_info, udata, dxpl_id) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_WRITEERROR, NULL,
    ...
    H5O_copy_t* cpy_info)
{
    ...
    H5O_shared_t* shared_dst =
        (H5O_shared_t*)mesg_dst; /* Alias to shared info in native destination */
    herr_t ret_value = SUCCEED;  /* Return value */
...
#error "Need to define H5O_SHARED_POST_COPY_FILE macro!"
    ...
    if(H5O_SHARED_POST_COPY_FILE_REAL(
           oloc_src, mesg_src, oloc_dst, mesg_dst, dxpl_id, cpy_info) < 0)
        HGOTO_ERROR(
            H5E_OHDR, H5E_CANTCOPY, FAIL, "unable to copy native message to another file")
    ...
    if(H5O_shared_post_copy_file(oloc_dst->file, H5O_SHARED_TYPE, shared_src, shared_dst,
           mesg_flags, dxpl_id, cpy_info) < 0)
        HGOTO_ERROR(
            H5E_OHDR, H5E_WRITEERROR, FAIL, "unable to fix shared message in post copy")

#ifdef H5O_SHARED_POST_COPY_FILE_UPD
    /* Call native message's post copy file update callback to update the
     * message */
    if(H5O_SHARED_POST_COPY_FILE_UPD(
           oloc_src, mesg_src, oloc_dst, mesg_dst, dxpl_id, cpy_info) < 0)
...
#error "Need to define H5O_SHARED_DEBUG_REAL macro!"
    ...
    if(H5O_IS_STORED_SHARED(sh_mesg->type)) {
        ...
        if(H5O_shared_debug(sh_mesg, stream, indent, fwidth) < 0)
            HGOTO_ERROR(
                H5E_OHDR, H5E_WRITEERROR, FAIL, "unable to display shared message info")
    } /* end if */

    /* Call native message's debug callback */
    if(H5O_SHARED_DEBUG_REAL(f, dxpl_id, _mesg, stream, indent, fwidth) < 0)
        HGOTO_ERROR(
            H5E_OHDR, H5E_WRITEERROR, FAIL, "unable to display native message info")
