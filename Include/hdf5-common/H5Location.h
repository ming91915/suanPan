...
public:
    ...
    // H5R_OBJECT, const PropList& plist = PropList::DEFAULT);
    ...
    void
    getObjinfo(const H5std_string& name, hbool_t follow_link, H5G_stat_t& statbuf) const;
    ...
    // void mount(const char* name, H5File& child, PropList& plist) const; // removed from
    // 1.8.18 and 1.10.1
    void
    mount(const H5std_string& name, const H5File& child, const PropList& plist) const;
...
protected:
    ...
    void
    p_reference(void* ref, const char* name, hid_t space_id, H5R_type_t ref_type) const;
