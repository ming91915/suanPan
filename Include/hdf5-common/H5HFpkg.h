...
#define H5HF_MAN_INDIRECT_CHILD_DIR_ENTRY_SIZE(h)                                  \
    (((h)->filter_len > 0 ?                                                        \
            ((unsigned)(h)->sizeof_addr + (unsigned)(h)->sizeof_size +             \
                (unsigned)4) : /* Size of entries for filtered direct blocks */    \
            (unsigned)(h)                                                          \
                ->sizeof_addr) /* Size of entries for un-filtered direct blocks */ \
        )
    ...
    (/* General metadata fields */                                                   \
        ...
        + (MIN(r, (h)->man_dtable.max_direct_rows) * (h)->man_dtable.cparam.width *  \
              H5HF_MAN_INDIRECT_CHILD_DIR_ENTRY_SIZE(                                \
                  h)) /* Size of entries for direct blocks */                        \
        + (((r > (h)->man_dtable.max_direct_rows) ?                                  \
                   (r - (h)->man_dtable.max_direct_rows) :                           \
                   0) *                                                              \
              (h)->man_dtable.cparam.width *                                         \
...
#define H5HF_FSPACE_SECT_SINGLE \
    0 /* Section is a range of actual bytes in a direct block */
...
typedef struct H5HF_dtable_t {
    ...
    unsigned
        start_bits; /* # of bits for starting block size (i.e. log2(start_block_size)) */
    unsigned max_direct_bits;      /* # of bits for max. direct block size (i.e.
                                      log2(max_direct_size)) */
...
typedef struct H5HF_free_section_t {
    H5FS_section_info_t
        sect_info; /* Free space section information (must be first in struct) */
    union {
        struct {
            H5HF_indirect_t*
        ...
        struct {
            struct H5HF_free_section_t*
                under;            /* Pointer to indirect block underlying row section */
            ...
            hbool_t
                checked_out; /* Flag to indicate that a row section is temporarily out of
        ...
        struct {
            ...
            unsigned
                iblock_entries; /* Number of entries in indirect block where section is
                                   located */
            unsigned
                rc; /* Reference count of outstanding row & child indirect sections */
            unsigned dir_nrows; /* Number of direct rows in section */
            struct H5HF_free_section_t**
                dir_rows;         /* Array of pointers to outstanding row sections */
...
typedef struct H5HF_hdr_t {
    ...
    unsigned pline_root_direct_filter_mask; /* I/O filter mask for filtered root direct
                                               block */
    ...
    unsigned root_iblock_flags; /* Flags to indicate whether root indirect block is
                                   pinned/protected */
...
struct H5HF_indirect_t {
    ...
    struct H5HF_indirect_t* fd_parent; /* Saved copy of the parent pointer -- this   */
    ...
    struct H5HF_indirect_t**
        child_iblocks; /* Array of pointers to pinned child indirect blocks */
    ...
    H5HF_indirect_filt_ent_t*
        filt_ents; /* Pointer to filtered information for direct blocks */
};
...
typedef struct H5HF_direct_t {
    ...
    hsize_t file_size;  /* Size of direct block in file (only valid when block's space is
                           being freed) */
...
H5_DLL herr_t H5HF_hdr_start_iter(H5HF_hdr_t* hdr,
    ...
    hsize_t curr_off,
    unsigned curr_entry);
H5_DLL herr_t H5HF_hdr_skip_blocks(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    H5HF_indirect_t* iblock,
    unsigned start_entry,
    unsigned nentries);
H5_DLL herr_t H5HF_hdr_update_iter(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
...
H5_DLL herr_t H5HF_man_iblock_root_create(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    size_t min_dblock_size);
H5_DLL herr_t H5HF_man_iblock_root_double(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    size_t min_dblock_size);
H5_DLL herr_t H5HF_man_iblock_alloc_row(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    H5HF_free_section_t** sec_node);
H5_DLL herr_t H5HF_man_iblock_create(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    ...
    unsigned max_rows,
    haddr_t* addr_p);
H5_DLL H5HF_indirect_t* H5HF_man_iblock_protect(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    haddr_t iblock_addr,
    ...
    unsigned flags,
    hbool_t* did_protect);
H5_DLL herr_t H5HF_man_iblock_unprotect(H5HF_indirect_t* iblock,
    hid_t dxpl_id,
...
H5_DLL herr_t H5HF_man_iblock_attach(H5HF_indirect_t* iblock,
    unsigned entry,
    haddr_t dblock_addr);
H5_DLL herr_t H5HF_man_iblock_detach(H5HF_indirect_t* iblock,
    hid_t dxpl_id,
    unsigned entry);
H5_DLL herr_t H5HF_man_iblock_entry_addr(H5HF_indirect_t* iblock,
    unsigned entry,
    haddr_t* child_addr);
H5_DLL herr_t H5HF_man_iblock_delete(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    haddr_t iblock_addr,
    unsigned iblock_nrows,
    H5HF_indirect_t* par_iblock,
    unsigned par_entry);
H5_DLL herr_t H5HF_man_iblock_size(H5F_t* f,
    hid_t dxpl_id,
    H5HF_hdr_t* hdr,
    haddr_t iblock_addr,
    unsigned nrows,
...
H5_DLL herr_t H5HF_man_dblock_new(H5HF_hdr_t* fh,
    hid_t dxpl_id,
    size_t request,
    H5HF_free_section_t** ret_sec_node);
H5_DLL herr_t H5HF_man_dblock_create(hid_t dxpl_id,
    H5HF_hdr_t* hdr,
    H5HF_indirect_t* par_iblock,
    unsigned par_entry,
    haddr_t* addr_p,
    H5HF_free_section_t** ret_sec_node);
H5_DLL herr_t H5HF_man_dblock_destroy(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    H5HF_direct_t* dblock,
    haddr_t dblock_addr);
H5_DLL H5HF_direct_t* H5HF_man_dblock_protect(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    haddr_t dblock_addr,
    size_t dblock_size,
    H5HF_indirect_t* par_iblock,
    unsigned par_entry,
    unsigned flags);
H5_DLL herr_t H5HF_man_dblock_locate(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    hsize_t obj_off,
    H5HF_indirect_t** par_iblock,
    unsigned* par_entry,
    ...
    unsigned flags);
H5_DLL herr_t H5HF_man_dblock_delete(H5F_t* f,
    hid_t dxpl_id,
    haddr_t dblock_addr,
    hsize_t dblock_size);
...
H5_DLL herr_t H5HF_man_insert(H5HF_hdr_t* fh,
    hid_t dxpl_id,
    size_t obj_size,
    const void* obj,
    void* id);
...
H5_DLL herr_t H5HF_man_write(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    const uint8_t* id,
    const void* obj);
H5_DLL herr_t H5HF_man_op(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    const uint8_t* id,
    H5HF_operator_t op,
    void* op_data);
...
H5_DLL herr_t
H5HF_huge_insert(H5HF_hdr_t* hdr, hid_t dxpl_id, size_t obj_size, void* obj, void* id);
H5_DLL herr_t H5HF_huge_get_obj_len(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    const uint8_t* id,
    size_t* obj_len_p);
...
H5_DLL herr_t H5HF_huge_write(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    const uint8_t* id,
    const void* obj);
H5_DLL herr_t H5HF_huge_op(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    const uint8_t* id,
    H5HF_operator_t op,
    void* op_data);
...
H5_DLL herr_t H5HF_man_iter_start_offset(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
...
H5_DLL H5HF_free_section_t* H5HF_sect_single_new(hsize_t sect_off,
    ...
    unsigned par_entry);
H5_DLL herr_t H5HF_sect_single_revive(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    H5HF_free_section_t* sect);
H5_DLL herr_t H5HF_sect_single_dblock_info(H5HF_hdr_t* hdr,
    ...
    haddr_t* dblock_addr,
    size_t* dblock_size);
H5_DLL herr_t H5HF_sect_single_reduce(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    H5HF_free_section_t* sect,
    size_t amt);
H5_DLL herr_t H5HF_sect_row_revive(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    H5HF_free_section_t* sect);
H5_DLL herr_t H5HF_sect_row_reduce(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    H5HF_free_section_t* sect,
    unsigned* entry_p);
H5_DLL H5HF_indirect_t* H5HF_sect_row_get_iblock(H5HF_free_section_t* sect);
H5_DLL herr_t H5HF_sect_indirect_add(H5HF_hdr_t* hdr,
    hid_t dxpl_id,
    H5HF_indirect_t* iblock,
    unsigned start_entry,
    unsigned nentries);
