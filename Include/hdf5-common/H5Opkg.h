...
#define H5O_MIN_SIZE                                                               \
    22 /* Min. obj header data size (must be big enough for a message prefix and a \
...
#define H5O_ALIGN_F(F, X)                                                      \
    H5O_ALIGN_VERS(                                                            \
        (H5F_USE_LATEST_FLAGS(F, H5F_LATEST_OBJ_HEADER) ? H5O_VERSION_LATEST : \
                                                          H5O_VERSION_1),      \
        X)
...
#define H5O_SIZEOF_MSGHDR_F(F, C)                                                      \
    H5O_SIZEOF_MSGHDR_VERS(                                                            \
        (H5F_USE_LATEST_FLAGS(F, H5F_LATEST_OBJ_HEADER) || H5F_STORE_MSG_CRT_IDX(F)) ? \
            H5O_VERSION_LATEST :                                                       \
            H5O_VERSION_1,                                                             \
        (C))
...
#define H5O_LOAD_NATIVE(F, DXPL, IOF, OH, MSG, ERR)                                     \
    if(NULL == (MSG)->native) {                                                         \
        ...
        if(NULL == ((MSG)->native = (msg_type->decode)(                                 \
                        (F), (DXPL), (OH), (MSG)->flags, &ioflags, (MSG)->raw)))        \
        ...
        if((MSG)->flags & H5O_MSG_FLAG_SHAREABLE) {                                     \
            ...
            H5O_UPDATE_SHARED((H5O_shared_t*)(MSG)->native, H5O_SHARE_TYPE_HERE, (F),   \
                msg_type->id, (MSG)->crt_idx, (OH)->chunk[0].addr)                      \
        } /* end if */                                                                  \
                                                                                        \
...
struct H5O_msg_class_t {
    ...
    herr_t (*del)(H5F_t*,
        hid_t,
        H5O_t*,
        void*); /* Delete space in file referenced by this message */
    herr_t (*link)(H5F_t*,
        hid_t,
        H5O_t*,
    ...
    herr_t (*post_copy_file)(const H5O_loc_t*,
        const void*,
        H5O_loc_t*,
        void*,
        unsigned*,
    ...
    herr_t (*get_crt_index)(const void*,
        H5O_msg_crt_idx_t*);                           /* Get message's creation index */
...
typedef struct H5O_msg_alloc_info_t {
    ...
    size_t
        null_size; /* Size of any null message in the chunk immediately after message */
...
typedef struct H5O_chunk_t {
    ...
    struct H5O_chunk_proxy_t*
        chunk_proxy; /* Pointer to a chunk's proxy when chunk protected */
} H5O_chunk_t;
...
/* Debugging information (not stored) */
    ...
    hbool_t store_bad_mesg_count; /* Flag to indicate that a bad message count should be
                                     stored */
...
typedef struct H5O_obj_class_t {
    ...
    hid_t (
        *open)(const H5G_loc_t*, hid_t, hid_t, hbool_t); /*open an object of this class */
    ...
    herr_t (*bh_info)(const H5O_loc_t* loc,
        hid_t dxpl_id,
        H5O_t* oh,
        H5_ih_info_t* bh_info); /*get the index & heap info for an object */
    herr_t (
        *flush)(void* obj_ptr, hid_t dxpl_id); /*flush an opened object of this class */
} H5O_obj_class_t;
...
typedef struct H5O_chunk_proxy_t {
    ...
    unsigned cont_chunkno; /* Chunk number for the chunk containing the continuation
                              message that points to this chunk */
...
H5_DLL herr_t
H5O_chunk_add(H5F_t* f, hid_t dxpl_id, H5O_t* oh, unsigned idx, unsigned cont_chunkno);
H5_DLL H5O_chunk_proxy_t*
...
H5_DLL herr_t H5O_attr_dense_info_test(hid_t oid,
    hsize_t* name_count,
...
H5_DLL herr_t H5O_msg_get_chunkno_test(hid_t oid, unsigned msg_type, unsigned* chunk_num);
