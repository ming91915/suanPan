...
#define H5O_ALIGN_F(F, X) H5O_ALIGN_VERS((H5F_USE_LATEST_FLAGS(F, H5F_LATEST_OBJ_HEADER) ? H5O_VERSION_LATEST : H5O_VERSION_1), X)
...
#define H5O_SIZEOF_MSGHDR_F(F, C) H5O_SIZEOF_MSGHDR_VERS((H5F_USE_LATEST_FLAGS(F, H5F_LATEST_OBJ_HEADER) || H5F_STORE_MSG_CRT_IDX(F)) ? H5O_VERSION_LATEST : H5O_VERSION_1, (C))
...
#define H5O_LOAD_NATIVE(F, DXPL, IOF, OH, MSG, ERR)                                                                                                                                    \
    if(NULL == (MSG)->native) {                                                                                                                                                        \
        ...
        if(NULL == ((MSG)->native = (msg_type->decode)((F), (DXPL), (OH), (MSG)->flags, &ioflags, (MSG)->raw))) HGOTO_ERROR(H5E_OHDR, H5E_CANTDECODE, ERR, "unable to decode message") \
                                                                                                                                                                                       \
        ...
        if(msg_type->set_crt_index) {                                                                                                                                                  \
            ...
            if((msg_type->set_crt_index)((MSG)->native, (MSG)->crt_idx) < 0) HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, ERR, "unable to set creation index")                                   \
        } /* end if */                                                                                                                                                                 \
    }     /* end if */
...
H5_DLL herr_t H5O_chunk_add(H5F_t* f, hid_t dxpl_id, H5O_t* oh, unsigned idx, unsigned cont_chunkno);
...
H5_DLL herr_t H5O__alloc_chunk(H5F_t* f, hid_t dxpl_id, H5O_t* oh, size_t size, size_t found_null, const H5O_msg_alloc_info_t* found_msg, size_t* new_idx);
