...
#define H5EA_IBLOCK_SIZE(i)                                                          \
    (/* General metadata fields */                                                   \
        ...
        + ((i)->ndblk_addrs *                                                        \
              (i)->hdr->sizeof_addr) /* Data block addresses in index block  */      \
        + ((i)->nsblk_addrs *                                                        \
              (i)->hdr->sizeof_addr) /* Super block addresses in index block  */     \
        )
...
#define H5EA_DBLOCK_SIZE(d)                                                         \
    (/* Data block prefix size  */                                                  \
        ...
        + ((d)->nelmts *                                                            \
              (size_t)(d)->hdr->cparam.raw_elmt_size) /* Elements in data block  */ \
...
#define H5EA_DBLK_PAGE_SIZE(h)                                                \
    (+((h)->dblk_page_nelmts *                                                \
         (size_t)(h)->cparam.raw_elmt_size) /* Elements in data block page */ \
...
typedef struct H5EA_sblock_t {
    ...
    size_t dblk_nelmts; /* # of elements for data blocks reachable through this super
                           block */
...
typedef struct H5EA_dblk_page_cache_ud_t {
    H5EA_hdr_t* hdr; /* Shared extensible array information */
    H5EA_sblock_t*
        parent; /* Pointer to parent object for data block page (super block) */
    haddr_t dblk_page_addr; /* Address of data block page */
} H5EA_dblk_page_cache_ud_t;

#ifdef H5EA_TESTING
typedef struct H5EA__ctx_cb_t {
    herr_t (*encode)(const void* elmt,
        size_t nelmts,
        void* udata); /* Perform action during encode step */
...
H5_DLL int H5EA_cmp_cparam_test(const H5EA_create_t* cparam1,
