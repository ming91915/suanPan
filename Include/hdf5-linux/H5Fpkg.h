...
#define H5F_SUPERBLOCK_VARLEN_SIZE(v, sizeof_addr, sizeof_size) ((v == 0 ? H5F_SUPERBLOCK_VARLEN_SIZE_V0(sizeof_addr, sizeof_size) : 0) + (v == 1 ? H5F_SUPERBLOCK_VARLEN_SIZE_V1(sizeof_addr, sizeof_size) : 0) + (v >= 2 ? H5F_SUPERBLOCK_VARLEN_SIZE_V2(sizeof_addr) : 0))
...
H5_DLL herr_t H5F__super_read(H5F_t* f, hid_t meta_dxpl_id, hid_t raw_dxpl_id, hbool_t initial_read);
...
H5_DLL herr_t H5F_super_ext_write_msg(H5F_t* f, hid_t dxpl_id, unsigned id, void* mesg, hbool_t may_create, unsigned mesg_flags);
...
H5_DLL herr_t H5F__accum_read(const H5F_io_info2_t* fio_info, H5FD_mem_t type, haddr_t addr, size_t size, void* buf);
H5_DLL herr_t H5F__accum_write(const H5F_io_info2_t* fio_info, H5FD_mem_t type, haddr_t addr, size_t size, const void* buf);
H5_DLL herr_t H5F__accum_free(const H5F_io_info2_t* fio_info, H5FD_mem_t type, haddr_t addr, hsize_t size);
...
H5_DLL htri_t H5F_try_extend(H5F_t* f, hid_t dxpl_id, H5FD_mem_t type, haddr_t blk_end, hsize_t extra_requested);
