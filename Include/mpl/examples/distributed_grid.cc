...
template <std::size_t dim, typename T, typename A> void update_overlap(const mpl::cart_communicator& C, mpl::distributed_grid<dim, T, A>& G, int tag = 0) {
    ...
    for(std::size_t i = 0; i < dim; ++i) {
        // send to left
        ranks = C.shift(i, -1);
        C.sendrecv(G.data(), G.left_border_layout(i), ranks.dest, tag, G.data(), G.right_mirror_layout(i), ranks.source, tag);
        // send to right
        ranks = C.shift(i, +1);
        C.sendrecv(G.data(), G.right_border_layout(i), ranks.dest, tag, G.data(), G.left_mirror_layout(i), ranks.source, tag);
    }
}
...
int main() {
    const mpl::communicator& comm_world(mpl::environment::comm_world());
    {
        ...
        for(auto i = G.obegin(0), i_end = G.oend(0); i < i_end; ++i) G(i) = comm_c.rank();
        ...
        for(int k = 0; k < comm_c.size(); ++k) {
            if(k == comm_c.rank()) {
                for(auto i = G.obegin(0), i_end = G.oend(0); i < i_end; ++i) std::cout << G(i);
                std::cout << std::endl;
            }
    ...
    }
    {
        ...
        for(auto j = G.obegin(1), j_end = G.oend(1); j < j_end; ++j)
            for(auto i = G.obegin(0), i_end = G.oend(0); i < i_end; ++i) G(i, j) = comm_c.rank();
        // get shadow data from adjacent processes
        update_overlap(comm_c, G);
        // print local grid including shadow grid points
        for(int k = 0; k < comm_c.size(); ++k) {
            if(k == comm_c.rank()) {
                std::cout << std::endl;
                for(auto j = G.obegin(1), j_end = G.oend(1); j < j_end; ++j) {
                    for(auto i = G.obegin(0), i_end = G.oend(0); i < i_end; ++i) std::cout << G(i, j);
                    std::cout << std::endl;
                }
            }
            comm_c.barrier(); // barrier may avoid overlapping output
