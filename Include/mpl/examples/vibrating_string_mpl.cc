...
int main() {
    ...
    for(double t = 2 * dt; t <= t_end; t += dt) {
        ...
        // update border data
        mpl::irequest_pool r;
        r.push(comm_world.isend(u_new_l[N_l[C_rank] - 2], C_rank + 1 < C_size ? C_rank + 1 : mpl::environment::proc_null(), right_copy));
        r.push(comm_world.isend(u_new_l[1], C_rank - 1 >= 0 ? C_rank - 1 : mpl::environment::proc_null(), left_copy));
        r.push(comm_world.irecv(u_new_l[0], C_rank - 1 >= 0 ? C_rank - 1 : mpl::environment::proc_null(), right_copy));
        r.push(comm_world.irecv(u_new_l[N_l[C_rank] - 1], C_rank + 1 < C_size ? C_rank + 1 : mpl::environment::proc_null(), left_copy));
        r.waitall();
    ...
    if(C_rank == 0) {
        ...
        for(int i = 0; i < N; ++i) std::cout << dx * i << '\t' << u[i] << '\n';
    } else
