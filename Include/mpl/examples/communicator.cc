...
int main() {
    ...
    std::cout << "comm_world  rank: " << comm_world.rank() << "\tsize: " << comm_world.size() << std::endl;
    comm_world.barrier();
    ...
    int key;
    if(comm_3.rank() == 0) key = comm_world.rank() % 3;
    comm_3.bcast(0, key);
    std::cout << "comm_3     rank: " << comm_3.rank() << "\tsize: " << comm_3.size() << "\tkey: " << key << std::endl;
    comm_world.barrier();
    ...
    mpl::communicator comm_without_0(mpl::communicator::split(), comm_world, comm_world.rank() == 0 ? mpl::environment::undefined() : 1);
    ...
    comm_world.barrier();
    std::cout << "comm_world  rank: " << comm_world.rank() << "\tcomm valid: " << (comm_without_0.is_valid() ? "yes" : "no") << std::endl;
    return EXIT_SUCCESS;
}
