...
void string(double* u, double* u_old, double* u_new, int N, double eps) {
    u_new[0] = u[0];
    for(int i = 1; i < N - 1; ++i) u_new[i] = eps * (u[i - 1] + u[i + 1]) + 2.0 * (1.0 - eps) * u[i] - u_old[i];
    u_new[N - 1] = u[N - 1];
}
...
int main(int argc, char* argv[]) {
    ...
    double dx = L / (N - 1), eps = dt * dt * c * c / (dx * dx), *u = NULL, *u_l, *u_old_l, *u_new_l, *u_temp;
    ...
    // local grids include one mirror grid point at each end or the bondary condition
    ...
    for(double t = 2 * dt; t <= t_end; t += dt) {
        ...
        MPI_Isend(&u_new_l[N_l[C_rank] - 2], 1, MPI_DOUBLE, C_rank + 1 < C_size ? C_rank + 1 : MPI_PROC_NULL, right_copy, MPI_COMM_WORLD, &requests[0]);
        MPI_Isend(&u_new_l[1], 1, MPI_DOUBLE, C_rank - 1 >= 0 ? C_rank - 1 : MPI_PROC_NULL, left_copy, MPI_COMM_WORLD, &requests[1]);
        MPI_Irecv(&u_new_l[0], 1, MPI_DOUBLE, C_rank - 1 >= 0 ? C_rank - 1 : MPI_PROC_NULL, right_copy, MPI_COMM_WORLD, &requests[2]);
        MPI_Irecv(&u_new_l[N_l[C_rank] - 1], 1, MPI_DOUBLE, C_rank + 1 < C_size ? C_rank + 1 : MPI_PROC_NULL, left_copy, MPI_COMM_WORLD, &requests[3]);
    ...
    if(C_rank == 0) u = secure_malloc(N * sizeof(*u));
