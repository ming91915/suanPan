...
template <typename ch, typename tr, typename T1, typename T2> std::basic_ostream<ch, tr>& operator<<(std::basic_ostream<ch, tr>& out, const std::pair<T1, T2>& p) { return out << '(' << p.first << ',' << p.second << ')'; }

// helper function for printing all elements of a tuple/ an array
template <typename ch, typename tr, typename tuple, std::size_t... is> void print_tuple_impl(std::basic_ostream<ch, tr>& out, const tuple& t, std::index_sequence<is...> is_) {
    auto print_element = [&](auto i, auto a) -> void { out << a << (i + 1 < is_.size() ? "," : ""); };
    std::initializer_list<int>{ (print_element(is, std::get<is>(t)), 0)... };
}

// print all elements of a tuple
template <typename ch, typename tr, typename... args> std::basic_ostream<ch, tr>& operator<<(std::basic_ostream<ch, tr>& out, const std::tuple<args...>& t) {
    out << '(';
...
template <typename ch, typename tr, typename ty, std::size_t s> std::basic_ostream<ch, tr>& operator<<(std::basic_ostream<ch, tr>& out, const std::array<ty, s>& t) {
    out << '(';
...
// print a byte
template <typename ch, typename tr> std::basic_ostream<ch, tr>& operator<<(std::basic_ostream<ch, tr>& out, const std::byte& t) { return out << std::to_integer<int>(t); }
#endif

// send some item of a standard type
template <typename T> void send(const mpl::communicator& comm, const T& x) { comm.send(x, 1); }

// receive some item of a standard type
template <typename T> void recv(const mpl::communicator& comm) {
    T x;
    comm.recv(x, 0);
