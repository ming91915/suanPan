...
template <typename T> struct min {
    T operator()(const T& x, const T& y) { return !(y < x) ? x : y; }
};
...
template <typename T> struct logical_and {
    T operator()(const T& x, const T& y) { return x and y; }
};

template <typename T> struct logical_or {
    T operator()(const T& x, const T& y) { return x or y; }
};

template <typename T> struct logical_xor {
    T operator()(const T& x, const T& y) { return x xor y; }
};
...
namespace detail {
    ...
    template <typename T> struct get_result_type_impl;
    template <typename R, typename... A> struct get_result_type_impl<R(A...)> { using type = R; };
    template <typename T> using get_result_type = typename get_result_type_impl<T>::type;

    template <typename T> struct get_first_argument_type_impl;
    template <typename R, typename A1, typename... A> struct get_first_argument_type_impl<R(A1, A...)> { using type = A1; };
    template <typename T> using get_first_argument_type = typename get_first_argument_type_impl<T>::type;

    template <typename T> struct get_second_argument_type_impl;
    template <typename R, typename A1, typename A2, typename... A> struct get_second_argument_type_impl<R(A1, A2, A...)> { using type = A2; };
    ...
    template <typename T, typename F> class op {
    public:
        typedef F functor;
        ...
        static_assert(std::is_assignable<typename std::add_lvalue_reference<first_argument_type>::type, T>::value and std::is_assignable<typename std::add_lvalue_reference<second_argument_type>::type, T>::value and std::is_assignable<T&, result_type>::value, "argument type mismatch");
        ...
        static void apply(void* invec, void* inoutvec, int* len, MPI_Datatype* datatype) {
            ...
            for(int i = 0, i_end = *len; i < i_end; ++i, ++i1, ++i2) *i2 = local_f(*i1, *i2);
        }
        MPI_Op mpi_op;
        op() { MPI_Op_create(op::apply, op_traits<functor>::is_commutative(), &mpi_op); }
        ~op() { MPI_Op_free(&mpi_op); }
        static constexpr bool is_s_commutative() { return op_traits<functor>::is_commutative(); }
        op(op const&) = delete;
        void operator=(op const&) = delete;
    };

    template <typename T, typename F> F* op<T, F>::f;

    template <typename T, typename F> op<T, F>& get_op() {
