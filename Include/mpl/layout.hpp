...
template <typename T> class null_layout;

template <typename T> class empty_layout;

template <typename T> class contiguous_layout;

template <typename T> class vector_layout;

template <typename T> class strided_vector_layout;

template <typename T> class indexed_layout;

template <typename T> class hindexed_layout;

template <typename T> class indexed_block_layout;

template <typename T> class hindexed_block_layout;

template <typename T> class iterator_layout;

template <typename T> class subarray_layout;

class heterogeneous_layout;

template <typename T> std::pair<T*, MPI_Datatype> data_layout(T*, const layout<T>&);
...
protected:
    explicit layout(MPI_Datatype new_type)
        : type(new_type) {
        if(type != MPI_DATATYPE_NULL) MPI_Type_commit(&type);
    }
...
template <typename T> class null_layout : public layout<T> {
    using layout<T>::type;
...
template <typename T> class empty_layout : public layout<T> {
    using layout<T>::type;
...
template <typename T> class contiguous_layout : public layout<T> {
    using layout<T>::type;
    static MPI_Datatype build(int count, MPI_Datatype old_type = datatype_traits<T>::get_datatype()) {
        MPI_Datatype new_type;
...
template <typename T> class vector_layout : public layout<T> {
    using layout<T>::type;
    static MPI_Datatype build(int count, MPI_Datatype old_type = datatype_traits<T>::get_datatype()) {
        MPI_Datatype new_type;
        MPI_Type_contiguous(count, old_type, &new_type);
...
template <typename T> class strided_vector_layout : public layout<T> {
    using layout<T>::type;
    static MPI_Datatype build() {
        MPI_Datatype new_type;
        MPI_Type_contiguous(0, datatype_traits<T>::get_datatype(), &new_type);
    ...
    static MPI_Datatype build(int count, int blocklength, int stride, MPI_Datatype old_type = datatype_traits<T>::get_datatype()) {
        MPI_Datatype new_type;
...
template <typename T> class indexed_layout : public layout<T> {
    using layout<T>::type;
...
public:
    ...
    public:
        parameter() = default;
        template <typename List_T> parameter(const List_T& V) {
            for(const auto& i : V) add(i[0], i[1]);
        }
        parameter(std::initializer_list<std::array<int, 2>> list) {
            for(const std::array<int, 2>& i : list) add(i[0], i[1]);
        }
...
private:
    ...
    static MPI_Datatype build(const parameter& par, MPI_Datatype old_type = datatype_traits<T>::get_datatype()) {
        MPI_Datatype new_type;
        MPI_Type_indexed(par.displacements.size(), par.blocklengths.data(), par.displacements.data(), old_type, &new_type);
        return new_type;
    }
...
template <typename T> class hindexed_layout : public layout<T> {
    using layout<T>::type;
...
public:
    ...
    public:
        parameter() = default;
        template <typename List_T> parameter(const List_T& V) {
            for(const auto& i : V) add(i.first, i.second);
        }
        parameter(std::initializer_list<std::pair<int, MPI_Aint>> list) {
            for(const std::pair<int, MPI_Aint>& i : list) add(i.first, i.second);
        }
...
private:
    ...
    static MPI_Datatype build(const parameter& par, MPI_Datatype old_type = datatype_traits<T>::get_datatype()) {
        MPI_Datatype new_type;
        MPI_Type_create_hindexed(par.displacements.size(), par.blocklengths.data(), par.displacements.data(), old_type, &new_type);
        return new_type;
    }
...
template <typename T> class indexed_block_layout : public layout<T> {
    using layout<T>::type;
...
private:
    ...
    static MPI_Datatype build(int blocklengths, const parameter& par, MPI_Datatype old_type = datatype_traits<T>::get_datatype()) {
        MPI_Datatype new_type;
        MPI_Type_create_indexed_block(par.displacements.size(), blocklengths, par.displacements.data(), old_type, &new_type);
        return new_type;
    }
...
template <typename T> class hindexed_block_layout : public layout<T> {
    using layout<T>::type;
...
private:
    ...
    static MPI_Datatype build(int blocklengths, const parameter& par, MPI_Datatype old_type = datatype_traits<T>::get_datatype()) {
        MPI_Datatype new_type;
        MPI_Type_create_hindexed_block(par.displacements.size(), blocklengths, par.displacements.data(), old_type, &new_type);
        return new_type;
    }
...
template <typename T> class iterator_layout : public layout<T> {
    using layout<T>::type;

public:
    class parameter {
        std::vector<MPI_Aint> displacements;
        template <typename value_T> void add(value_T& base, value_T*& i) { add(reinterpret_cast<char*>(&i) - reinterpret_cast<char*>(&base)); }
        template <typename value_T> void add(const value_T& base, const value_T& i) { add(reinterpret_cast<const char*>(&i) - reinterpret_cast<const char*>(&base)); }
        void add(MPI_Aint displacement) { displacements.push_back(displacement); }

    public:
        parameter() = default;
        template <typename iter_T> parameter(iter_T first, iter_T end) {
            for(iter_T i = first; i != end; ++i) add(*first, *i);
        }
...
private:
    ...
    static MPI_Datatype build(const parameter& par, MPI_Datatype old_type = datatype_traits<T>::get_datatype()) {
        MPI_Datatype new_type;
        MPI_Type_create_hindexed_block(par.displacements.size(), 1, par.displacements.data(), old_type, &new_type);
        return new_type;
    }
...
template <typename T> class subarray_layout : public layout<T> {
    using layout<T>::type;
...
public:
    ...
    public:
        parameter() = default;
        template <typename List_T> parameter(const List_T& V) {
            for(const auto& i : V) add(i[0], i[1], i[2]);
        }
        parameter(std::initializer_list<std::array<int, 3>> list) {
            for(const std::array<int, 3>& i : list) add(i[0], i[1], i[2]);
        }
...
private:
    ...
    static MPI_Datatype build(const parameter& par, MPI_Datatype old_type = datatype_traits<T>::get_datatype()) {
        MPI_Datatype new_type;
        int total_size = 1;
        for(std::vector<int>::size_type i = 0; i < par.sizes.size(); ++i) total_size *= par.subsizes[i];
        if(total_size > 0)
            MPI_Type_create_subarray(par.sizes.size(), par.sizes.data(), par.subsizes.data(), par.starts.data(), static_cast<int>(par.order()), old_type, &new_type);
        else
            new_type = build();
        return new_type;
    }
...
public:
    ...
    public:
        parameter() = default;
        template <typename... Ts> parameter(const Ts&... xs) { add(xs...); }
        template <typename T, typename... Ts> void add(const T& x, const Ts&... xs) {
        ...
        template <typename T, typename... Ts> void add(const std::pair<T*, MPI_Datatype>& x, const Ts&... xs) {
            blocklengths.push_back(1);
...
private:
    ...
    static MPI_Datatype build(const parameter& par) {
        MPI_Datatype new_type;
        MPI_Type_create_struct(static_cast<int>(par.blocklengths.size()), par.blocklengths.data(), par.displacements.data(), par.types.data(), &new_type);
        return new_type;
    }
...
template <typename T> inline std::pair<T*, MPI_Datatype> data_layout(T* x, const layout<T>& l) { return std::make_pair(x, l.type); }

//--------------------------------------------------------------------

template <typename T> struct datatype_traits<layout<T>> {
    static MPI_Datatype get_datatype(const layout<T>& l) { return l.type; }
};

//--------------------------------------------------------------------

template <typename T> class layouts : private std::vector<layout<T>> {
...
template <typename T> class contiguous_layouts : private std::vector<contiguous_layout<T>> {
...
public:
    ...
    const int* sizes() const {
        s.resize(0);
        for(const auto& i : *this) s.push_back(i.size());
        return s.data();
    }
};
}
