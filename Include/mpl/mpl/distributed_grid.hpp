...
template <std::size_t dim, typename T, typename A> class distributed_grid;

template <std::size_t dim, typename T, typename A> class local_grid;
...
private:
    ...
    vector_type v;
    std::vector<subarray_layout<T>> left_mirror_layout_, right_mirror_layout_, left_border_layout_, right_border_layout_;
    subarray_layout<T> interior_layout_;

    size_type gbegin(size_type n, int comm_size, int comm_coord) const { return n * comm_coord / comm_size; }
    size_type gend(size_type n, int comm_size, int comm_coord) const { return n * (comm_coord + 1) / comm_size; }
...
public:
    ...
    public:
        sizes(std::initializer_list<std::pair<size_type, size_type>> list) {
            for(const std::pair<size_type, size_type>& i : list) add(i.first, i.second);
        }
    ...
    distributed_grid(const cart_communicator& C, const sizes& size)
        ...
        if(C.dim() != dim or gsize_.size() != dim or overlap_.size() != dim) throw invalid_dim();
#endif
        auto C_size = C.dims();
    ...
    const subarray_layout<T>& left_mirror_layout(size_type i) const { return left_mirror_layout_[i]; }
    const subarray_layout<T>& right_mirror_layout(size_type i) const { return right_mirror_layout_[i]; }
    const subarray_layout<T>& left_border_layout(size_type i) const { return left_border_layout_[i]; }
    const subarray_layout<T>& right_border_layout(size_type i) const { return right_border_layout_[i]; }
...
public:
    ...
    public:
        sizes(std::initializer_list<size_type> list) {
            for(const size_type& i : list) add(i);
        }
    ...
    local_grid(const cart_communicator& C, const sizes& size)
        : gsize_(size.size_) {
#if defined MPL_DEBUG
        if(C.dim() != dim or gsize_.size() != dim) throw invalid_dim();
#endif
        size_type vol = 1;
        for(std::size_t i = 0; i < dim; ++i) vol *= gsize_[i];
        v.resize(vol);
        auto C_size = C.dims();
        for(int i = 0, i_end = C.size(); i < i_end; ++i) {
            ...
            for(std::size_t j = dim - 1; true; --j) {
                par.add(gsize_[j], gend(gsize_[j], C_size[j], coords[j]) - gbegin(gsize_[j], C_size[j], coords[j]), gbegin(gsize_[j], C_size[j], coords[j]));
                if(j == 0) break;
            }
