...
#define MPL_CHECK_DEST(dest)                                               \
    if (dest != environment::proc_null() and (dest < 0 or dest >= size())) \
        throw invalid_rank();

#define MPL_CHECK_SOURCE(source)                                                                                         \
    if (source != environment::proc_null() and source != environment::any_source() and (source < 0 or source >= size())) \
        throw invalid_rank();
...
#define MPL_CHECK_RTAG(tag)                                                         \
    if (tag != environment::any_tag() and (tag < 0 or tag > environment::tag_up())) \
        throw invalid_tag();
