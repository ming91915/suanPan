...
namespace environment {
    ...
    inline int tag_up() { return detail::get_env().tag_up(); }

    constexpr int any_tag() { return MPI_ANY_TAG; }

    constexpr int any_source() { return MPI_ANY_SOURCE; }

    constexpr int proc_null() { return MPI_PROC_NULL; }

    constexpr int undefined() { return MPI_UNDEFINED; }

    constexpr int root() { return MPI_ROOT; }

    constexpr void* absolute() { return reinterpret_cast<void*>(MPI_BOTTOM); }

    constexpr int bsend_overheadroot() { return MPI_BSEND_OVERHEAD; }
    ...
    inline void buffer_attach(void* buff, int size) { return detail::get_env().buffer_attach(buff, size); }
...
template <typename A = std::allocator<char>> class bsend_buffer {
    int size;
    A alloc;
    char* buff;
