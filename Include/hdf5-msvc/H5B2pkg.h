...
#define H5B2_INT_POINTER_SIZE(h, d)                                      \
    ((unsigned)(h)->sizeof_addr /* Address of child node */              \
        ...
        +                                                                \
        (h)->node_info[(d)-1]                                            \
            .cum_max_nrec_size /* Total # of records in child & below */ \
        )
...
#define H5B2_HEADER_SIZE(sizeof_addr, sizeof_size)                              \
    (/* General metadata fields */                                              \
        ...
        + H5B2_TREE_POINTER_SIZE(                                               \
              sizeof_addr, sizeof_size) /* Node pointer to root node in tree */ \
        )
...
typedef struct {
    ...
    uint8_t cum_max_nrec_size; /* Size to store cumulative max. # of records for this node
                                  (in bytes) */
...
typedef struct H5B2_hdr_t {
    ...
    H5B2_node_info_t*
        node_info;        /* Table of node info structs for current depth of B-tree */
...
typedef enum H5B2_update_status_t {
    ...
    H5B2_UPDATE_SHADOW_DONE,      /* Update modified existing record and modified node was
                                     shadowed */
...
H5_DLL herr_t H5B2_get_node_info_test(H5B2_t* bt2,
    hid_t dxpl_id,
    void* udata,
