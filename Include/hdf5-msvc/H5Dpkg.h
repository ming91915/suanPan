...
#define H5D_FARRAY_MAX_DBLK_PAGE_NELMTS_BITS \
    10 /* i.e. 1024 elements per data block page */
...
#define H5D_EARRAY_MAX_DBLOCK_PAGE_NELMTS_BITS \
    10 /* i.e. 1024 elements per data block page */
...
typedef struct H5D_type_info_t {
    ...
    const H5T_subset_info_t*
        cmpd_subset;       /* Info related to the compound subset conversion functions */
...
typedef herr_t (
    *H5D_layout_init_func_t)(H5F_t* f, hid_t dxpl_id, const H5D_t* dset, hid_t dapl_id);
...
typedef struct H5D_layout_ops_t {
    ...
    H5D_layout_is_space_alloc_func_t
        is_space_alloc; /* Query routine to determine if storage is allocated */
    H5D_layout_io_init_func_t io_init; /* I/O initialization routine */
    H5D_layout_read_func_t
        ser_read; /* High-level I/O routine for reading data in serial */
    H5D_layout_write_func_t
        ser_write; /* High-level I/O routine for writing data in serial */
#ifdef H5_HAVE_PARALLEL
    H5D_layout_read_func_t
        par_read; /* High-level I/O routine for reading data in parallel */
    H5D_layout_write_func_t
        par_write; /* High-level I/O routine for writing data in parallel */
...
typedef struct H5D_io_info_t {
    ...
    struct {
        ...
        H5FD_mpio_collective_opt_t
            coll_opt_mode; /* Parallel transfer with independent IO or collective IO with
                              this mode */
...
typedef herr_t (*H5D_chunk_copy_shutdown_func_t)(H5O_storage_chunk_t* storage_src,
    ...
    hid_t dxpl_id);
typedef herr_t (
    *H5D_chunk_size_func_t)(const H5D_chk_idx_info_t* idx_info, hsize_t* idx_size);
typedef herr_t (
    *H5D_chunk_reset_func_t)(H5O_storage_chunk_t* storage, hbool_t reset_addr);
...
typedef struct H5D_chunk_ops_t {
    ...
    H5D_chunk_resize_func_t
        resize; /* Routine to update chunk index info after resizing dataset */
    ...
    H5D_chunk_delete_func_t
        idx_delete; /* Routine to delete index & all chunks from file*/
    H5D_chunk_copy_setup_func_t
        copy_setup; /* Routine to perform any necessary setup for copying chunks */
...
typedef struct H5D_chunk_info_t {
    ...
    hsize_t scaled[H5O_LAYOUT_NDIMS]; /* Scaled coordinates of chunk (in file dataset's
                                         dataspace) */
    H5S_t* fspace;                    /* Dataspace describing chunk & selection in it */
    hbool_t fspace_shared; /* Indicate that the file space for a chunk is shared and
                              shouldn't be freed */
    H5S_t*
        mspace; /* Dataspace describing selection in memory corresponding to this chunk */
    hbool_t mspace_shared; /* Indicate that the memory space for a chunk is shared and
                              shouldn't be freed */
} H5D_chunk_info_t;
...
#ifdef H5_HAVE_PARALLEL
    H5D_chunk_info_t** select_chunk; /* Store the information about whether this chunk is
                                        selected or not */
#endif                               /* H5_HAVE_PARALLEL */
} H5D_chunk_map_t;
...
typedef struct H5D_rdcc_t {
    ...
    struct H5D_rdcc_ent_t* tmp_head; /* Head of temporary doubly linked list.  Chunks on
                                        this list are not in the hash table (slot).  The
                                        head entry is a sentinel (does not refer to an
                                        actual chunk). */
    ...
    hsize_t scaled_power2up[H5S_MAX_RANK]; /* The scaled dim sizes, rounded up to next
                                              power of 2 */
    unsigned scaled_encode_bits
        [H5S_MAX_RANK]; /* The number of bits needed to encode the scaled dim sizes */
} H5D_rdcc_t;
...
typedef struct H5D_shared_t {
    ...
    hsize_t curr_power2up
        [H5S_MAX_RANK]; /* The curr. dim sizes, rounded up to next power of 2 */
...
typedef struct H5D_fill_buf_info_t {
    ...
    H5T_path_t* fill_to_mem_tpath;   /* Datatype conversion path for converting the fill
                                        value to the memory buffer */
    H5T_path_t* mem_to_dset_tpath;   /* Datatype conversion path for converting the memory
    ...
    hbool_t has_vlen_fill_type;           /* Whether the datatype for the fill value has a
...
H5_DLL H5D_t* H5D__create_named(const H5G_loc_t* loc,
    ...
    hid_t dxpl_id);
H5_DLL H5D_t*
H5D__open_name(const H5G_loc_t* loc, const char* name, hid_t dapl_id, hid_t dxpl_id);
...
H5_DLL herr_t H5D__chunk_allocate(const H5D_io_info_t* io_info,
    hbool_t full_overwrite,
    hsize_t old_dim[]);
H5_DLL herr_t H5D__chunk_update_old_edge_chunks(H5D_t* dset,
    hid_t dxpl_id,
    hsize_t old_dim[]);
...
H5_DLL herr_t H5D__virtual_copy(H5F_t* f_src, H5O_layout_t* layout_dst, hid_t dxpl_id);
H5_DLL herr_t H5D__virtual_init(H5F_t* f,
    hid_t dxpl_id,
    const H5D_t* dset,
    hid_t dapl_id);
