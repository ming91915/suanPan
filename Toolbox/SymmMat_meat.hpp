...
template <typename eT>
SymmMat<eT>::SymmMat(const SizeMat& s)
    : n_size(s.n_rows)
...
template <typename eT>
template <typename fill_type>
SymmMat<eT>::SymmMat(const SizeMat& s, const fill::fill_class<fill_type>& f)
    : n_size(s.n_rows)
    , n_elem((s.n_rows + 1) * s.n_cols / 2)
    , mem_state(0)
    , mem()
...
template <typename eT>
SymmMat<eT>::SymmMat(const Mat<eT>& in_mat)
...
template <typename eT>
SymmMat<eT>::SymmMat(const SymmMat& m)
    : n_size(m.n_size)
    , n_elem(m.n_elem)
    , mem_state(0)
    , mem()
...
{
    arma_debug_check(
        in_row >= n_size || in_col >= n_size, "SymmMat::operator(): index out of bounds");
...
template <typename eT> void SymmMat<eT>::init_cold()
{
    ...
    const char* error_message =
        "SymmMat::init(): requested size is too large; suggest to "
...
template <typename eT> void SymmMat<eT>::init_warm(const uword& in_size)
{
    ...
    arma_debug_set_error(err_state, err_msg, t_mem_state == 3,
        "SymmMat::init(): size is fixed and hence cannot be changed");
