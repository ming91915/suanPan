...
int B21::update_status() {
    ...
    // transform global deformation to local one (remove rbm)
    vec t_disp(6);
    for(auto I = 0; I < 3; ++I) {
        t_disp(I) = disp_i(I);
        t_disp(I + 3) = disp_j(I);
    }
    vec local_deformation = trans_mat * t_disp;

    // used for interpolation
    vec t_deform(4, fill::zeros);
    t_deform(1) = local_deformation(1);
    t_deform(3) = local_deformation(2);

    // for each section first strain is normal strain
    vec section_deform(2);
    section_deform(0) = local_deformation(0) / new_length;

    for(const auto& I : int_pt) {
        // get curvature
        section_deform(1) = dot(shape::beam(I.coor, 1, new_length), t_deform);
        I.b_section->update_status(section_deform);
    }
