...
void new_cp3(unique_ptr<Element>& return_obj, istringstream& command)
{
    ...
    if(!command.eof()) {
        if((command >> thickness).fail())
            suanpan_debug("new_cp3() needs a valid thickness.\n");
    } else
...
void new_cp4(unique_ptr<Element>& return_obj, istringstream& command)
{
    ...
    if(!command.eof()) {
        if((command >> reduced_scheme).fail())
            suanpan_debug("new_cp4() needs a valid reduced integration switch (0,1).\n");
    } else
    ...
    if(!command.eof()) {
        if((command >> nonlinear).fail())
            suanpan_debug("new_cp4() needs a valid nonlinear geometry switch (0,1).\n");
    } else
...
void new_gq12(unique_ptr<Element>& return_obj, istringstream& command)
{
    ...
    if(!command.eof()) {
        if((command >> thickness).fail())
            suanpan_debug("new_gq12() needs a valid thickness.\n");
    } else
...
void new_ps(unique_ptr<Element>& return_obj, istringstream& command)
{
    ...
    if(!command.eof()) {
        if((command >> thickness).fail())
            suanpan_debug("new_ps() needs a valid thickness.\n");
    } else
...
void new_qe2(unique_ptr<Element>& return_obj, istringstream& command)
{
    ...
    if(!command.eof()) {
        if((command >> thickness).fail())
            suanpan_debug("new_qe2() needs a valid thickness.\n");
    } else
...
void new_truss2d(unique_ptr<Element>& return_obj, istringstream& command)
{
    ...
    if(!command.eof()) {
        if((command >> nonlinear).fail())
            suanpan_debug(
                "new_truss2d() needs a valid nonlinear geometry switch (0,1).\n");
    } else
    ...
    if(!command.eof()) {
        if((command >> update_area).fail())
            suanpan_debug(
                "new_truss2d() needs a valid switch (0,1) to indicate if update area.\n");
    } else
        suanpan_debug("new_truss2d() assumes constant area.\n");

    if(!command.eof()) {
        if((command >> log_strain).fail())
            suanpan_debug("new_truss2d() needs a valid switch (0,1) to indicate if to "
                          "use engineering strain.\n");
    } else
        suanpan_debug("new_truss2d() assumes engineering strain.\n");

    return_obj = make_unique<Truss2D>(tag, uvec(node_tag), material_tag, area,
        !!nonlinear, !!update_area, !!log_strain);
}
...
void new_c3d8(unique_ptr<Element>& return_obj, istringstream& command)
{
    ...
    if(!command.eof()) {
        if((command >> reduced_scheme).fail())
            suanpan_debug("new_c3d8() needs a valid reduced integration switch (0,1).\n");
    } else
    ...
    if(!command.eof()) {
        if((command >> nonlinear).fail())
            suanpan_debug("new_c3d8() needs a valid nonlinear geomtery switch (0,1).\n");
    } else
