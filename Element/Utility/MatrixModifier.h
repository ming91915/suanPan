...
namespace mass {
    ...
    template <typename T> void lumped_simple::apply(Mat<T>& mass) { mass = diagmat(sum(mass)); }
...
namespace damping {
    struct rayleigh {
        template <typename T> static void apply(const shared_ptr<Element>&, const double, const double);
    };
