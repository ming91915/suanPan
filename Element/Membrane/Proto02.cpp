...
mat Proto02::mapping;
mat Proto02::converter;

Proto02::IntegrationPoint::IntegrationPoint(const vec& C, const double W, const double J, unique_ptr<Material>&& M)
    : coor(C)
    , weight(W)
    , jacob_det(J)
    , m_material(move(M))
...
Proto02::Proto02(const unsigned T, const uvec& N, const unsigned M, const double TH, const bool RS)
...
int Proto02::update_status() {
    ...
    const mat t_mat = HILI.t() * HT;
    const mat QT = t_mat * HILI;                                 // eq. 60
    const mat TT = t_mat * HIL;                                  // eq. 60
